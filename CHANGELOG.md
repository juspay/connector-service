...
## [2025-01-18] - TPSL Connector Enhancement

### Added
- Enhanced TPSL connector implementation with comprehensive UPI support
- Payment methods supported: UPI (Unified Payments Interface) - Intent, Collect, Token Generation
- Transaction flows: Authorize, PSync (Payment Sync), CreateSessionToken (UPI Token Generation)
- Full UCS v2 macro framework compliance with zero manual trait implementations
- Complete migration from Haskell euler-api-txns preserving all business logic

### Files Created/Modified
- `src/connectors/tpsl.rs` - Main connector implementation with macro framework
- `src/connectors/tpsl/transformers.rs` - Enhanced request/response transformers with UPI flows
- `src/connectors/tpsl/constants.rs` - API constants and endpoints for UPI-specific flows
...
- **Mandatory UCS v2 Macro Framework**: Uses `create_all_prerequisites!` and `macro_connector_implementation!` exclusively
- **Zero Manual Code**: No manual trait implementations - all generated by macros
- **Haskell Parity**: Complete business logic preservation from original euler-api-txns implementation
- **Authentication**: Enhanced TPSL authentication with Bearer tokens and merchant codes
- **Type Safety**: Full guard rails with Secret<String> for sensitive data, proper amount handling
- **Error Handling**: Comprehensive error response mapping with TPSL-specific error codes
- **Environment Support**: Both test (tekprocess.co.in) and production (tpsl-india.in) endpoints

### UPI-Specific Implementation
- **UPI Intent Flow**: App-based UPI payments with proper token handling
- **UPI Collect Flow**: VPA-based payments with dynamic method configuration
- **Token Generation**: UPI token generation flow for session management
- **ACS Handling**: Access Control Server redirect support for 3DS flows
- **Status Mapping**: Proper mapping of TPSL transaction states to UCS enums
- **Error Codes**: TPSL-specific error handling (000=Success, 001/002=Failure, 003=Pending)

### Request/Response Structures
- **TpslPaymentsRequest**: Core payment request with merchant, cart, payment, transaction, consumer payloads
- **TpslUPITokenRequest**: UPI-specific token generation request
- **TpslUPITxnRequest**: UPI transaction request with intent/collect support
- **TpslCombinedTokenResponse**: Unified response handling for UPI token flows
- **Enhanced Error Types**: TpslErrorResponse, TpslErrorPayload for comprehensive error handling

### Authentication & Security
- **Multi-currency Support**: Currency-based authentication key mapping
- **Enhanced Headers**: Bearer token + merchant code + optional checksum
- **Secret Management**: Proper masking of sensitive authentication data
- **Dynamic Auth**: Runtime authentication extraction based on request currency

### Migration Achievements
- **Complete Feature Parity**: All Haskell features preserved and enhanced
- **Production Ready**: Full compilation success with comprehensive error handling
- **Type Safety**: Compile-time guarantees with proper domain types
- **Performance**: Optimized request/response transformations
- **Maintainability**: Clean macro-based implementation for easy updates
