...
- **connector:** [Payu] Complete UCS v2 connector implementation with UPI payment flows
  - **CRITICAL**: Fully migrated from manual trait implementations to mandatory UCS v2 macro framework
  ...
  - **MANDATORY**: Uses `create_all_prerequisites!` and `macro_connector_implementation!` macros - NO manual trait implementations
  - SHA-512 hash generation for PayU authentication following Haskell patterns
  ...
  - **COMPLIANCE**: All request values extracted dynamically from router data (no hardcoded values)
  - **TYPE SAFETY**: Proper guard rails with Secret<String>, MinorUnit, Email, Currency types

### Files Created/Modified
- `src/connectors/payu.rs` - **COMPLETE REWRITE**: Main connector implementation using UCS v2 macros only
- `src/connectors/payu/transformers.rs` - **UPDATED**: Request/response transformers with proper TryFrom implementations
- `src/connectors/payu/constants.rs` - API constants and endpoints (unchanged)
...
### Technical Details
- **MACRO FRAMEWORK**: Uses mandatory UCS v2 macro framework (`create_all_prerequisites!`, `macro_connector_implementation!`)
- **NO MANUAL CODE**: Zero manual trait implementations - all generated by macros
- **AMOUNT HANDLING**: Implements proper amount handling with `StringMajorUnit` converter
- **AUTHENTICATION**: Full authentication support with API key and merchant salt
- **ERROR HANDLING**: Comprehensive error handling with PayU-specific error codes
- **TYPE SAFETY**: Type-safe implementation with proper domain types and guard rails
- **DYNAMIC VALUES**: Dynamic value extraction from router data (no hardcoded values)
- **BASE64 SUPPORT**: Base64 decoding support for UPI Collect responses
- **STATUS MAPPING**: Complete status mapping from PayU to internal AttemptStatus
- **STUB FLOWS**: All unsupported flows have proper stub implementations for compilation
- **SOURCE VERIFICATION**: Complete source verification stubs for all flows
