...
def gen_js_types(flows: list[dict]) -> None:
    """Generate connector_client.d.ts so TypeScript/JS IDEs resolve flow methods."""
    lines = [
        "// AUTO-GENERATED — do not edit by hand.",
        "// Source: services.proto ∩ bindings/uniffi.rs  |  Regenerate: make generate",
        "//",
    ...
    for f in flows:
        ...
        lines.append(f"  /** {f['service']}.{f['rpc']} — {f['description']} */")
        lines.append(f"  {n}(")
        lines.append(f"    request: ucs.v2.I{req},")
        lines.append(f"    metadata: Metadata,")
        lines.append(f"    ffiOptions?: ucs.v2.IFfiOptions,")
    ...
    write(
        SDK_ROOT / "javascript/src/payments/connector_client.d.ts",
        "\n".join(lines) + "\n",
    )


def gen_uniffi_js_types(flows: list[dict]) -> None:
    """Generate uniffi_client.d.ts so IDEs resolve the low-level {flow}Req/{flow}Res methods."""
    lines = [
        "// AUTO-GENERATED — do not edit by hand.",
        "// Source: services.proto ∩ bindings/uniffi.rs  |  Regenerate: make generate",
        "//",
    ...
    for f in flows:
        ...
        lines.append(f"  /** {f['service']}.{f['rpc']} — {f['description']} */")
        lines.append(f"  {camel}Req(requestBytes: Bytes, metadata: Metadata, optionsBytes?: Bytes | null): string;")
        lines.append(f"  /** {f['service']}.{f['rpc']} — parse connector HTTP response */")
        lines.append(f"  {camel}Res(responseBody: Bytes, statusCode: number, responseHeaders: Metadata, requestBytes: Bytes, metadata: Metadata, optionsBytes?: Bytes | null): Buffer;")
        lines.append("")

    lines.append("}")

    write(
        SDK_ROOT / "javascript/src/payments/uniffi_client.d.ts",
        "\n".join(lines) + "\n",
    )
...
def gen_javascript(flows: list[dict]) -> None:
    ...
    for f in flows:
        lines.append(flow_comment(f, "  //"))
        lines.append(
            f'  {f["name"]:<30}: {{ request: "{f["request"]}", response: "{f["response"]}" }},'
        )
        lines.append("")
...
def gen_kotlin(flows: list[dict]) -> None:
    ...
    # FFI transformer imports
    for f in flows:
        n = f["name"]
        lines += [
            f"import uniffi.connector_service_ffi.{n}ReqTransformer",
            f"import uniffi.connector_service_ffi.{n}ResTransformer",
        ]
    lines.append("")
    ...
    for f in flows:
        lines.append(f'        "{f["name"]}" to ::{f["name"]}ReqTransformer,')
    lines += [
        "    )",
        "",
    ...
    for f in flows:
        lines.append(f'        "{f["name"]}" to ::{f["name"]}ResTransformer,')
    lines += ["    )", "}", ""]
