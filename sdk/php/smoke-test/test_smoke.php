...
{
    // Map connector name to Connector enum constant
    $connectorConstant = strtoupper($connectorName);
    $connectorEnumClass = \Types\Connector::class;
    if (!defined("{$connectorEnumClass}::{$connectorConstant}")) {
        throw new \InvalidArgumentException("Unknown connector: {$connectorName}");
    }
    $connectorValue = constant("{$connectorEnumClass}::{$connectorConstant}");

    // Build ConnectorAuth — map snake_case keys to camelCase proto fields
    $authFields = [];
    foreach ($authConfig as $key => $value) {
    ...
    // ConnectorAuth is a oneof keyed by connector name (lowercase)
    $connectorAuth = new \Types\ConnectorAuth();
    $setter = 'set' . ucfirst($connectorName);
    if (!method_exists($connectorAuth, $setter)) {
        throw new \InvalidArgumentException(
            "ConnectorAuth has no setter for connector '{$connectorName}'. "
    ...
    foreach ($authFields as $field => $value) {
        $authSetter = 'set' . ucfirst($field);
        if (method_exists($authMsg, $authSetter)) {
            // Handle SecretString wrapper
            if (is_array($value) && isset($value['value'])) {
                $secret = new \Types\SecretString();
                $secret->setValue((string) $value['value']);
                $authMsg->$authSetter($secret);
            } else {
                $authMsg->$authSetter($value);
            }
        }
    }

    $connectorAuth->$setter($authMsg);

    $config = new ConnectorConfig();
    $config->setConnector($connectorValue);
    $config->setEnvironment(Environment::SANDBOX);
    $config->setAuth($connectorAuth);
