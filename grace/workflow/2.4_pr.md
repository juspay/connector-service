...
| `{TECHSPEC_ISSUE_URL}`     | URL of the tracking tech spec issue in `juspay/grace` — **REQUIRED, non-empty** (hard gate)             | `https://github.com/juspay/grace/issues/42`                                                    |
| `{TECHSPEC_ISSUE_NUMBER}`  | Issue number in `juspay/grace` — **REQUIRED, non-empty** (hard gate)                                    | `42`                                                                                           |

---

## Phase 0: Tech Spec Issue Gate (HARD GATE — run FIRST)

Before doing any git work, validate the tech spec issue cross-reference. Without it, the PR cannot be created.

### 0a: Verify the inputs are present

If `{TECHSPEC_ISSUE_NUMBER}` is empty, zero, or non-numeric, OR if `{TECHSPEC_ISSUE_URL}` is empty, return STATUS=FAILED immediately with REASON=`"Tech spec issue gate failed: missing TECHSPEC_ISSUE_NUMBER/URL — the Tech Spec Agent must publish an issue in juspay/grace before the PR is created."`

Do NOT proceed to Phase 1. Do NOT commit. Do NOT push.

### 0b: Verify the issue actually exists in `juspay/grace`

```bash
gh issue view {TECHSPEC_ISSUE_NUMBER} --repo juspay/grace --json number,state,title --jq '.number'
```

If this command fails, returns nothing, or returns a number that does not match `{TECHSPEC_ISSUE_NUMBER}`, return STATUS=FAILED with REASON=`"Tech spec issue gate failed: juspay/grace#{TECHSPEC_ISSUE_NUMBER} could not be verified (<gh error>)."`

Only after this gate passes may you proceed to Phase 1.
...
**Tech spec issue cross-reference (MANDATORY in both templates)**: Insert the following block immediately after the `## Summary` section. The Phase 0 gate guarantees both variables are present and validated, so this section is unconditional:

```markdown
## Related Tech Spec

- Tracking issue: [juspay/grace#{TECHSPEC_ISSUE_NUMBER}]({TECHSPEC_ISSUE_URL}) — generated tech spec & source documentation used for this implementation.
```

If you ever find yourself about to skip or omit this block, STOP — Phase 0 was bypassed and the workflow is in an invalid state. Return STATUS=FAILED.
...
<Related Tech Spec block here, if applicable>
...
15. **Tech spec issue cross-reference is a HARD GATE** — `{TECHSPEC_ISSUE_NUMBER}` and `{TECHSPEC_ISSUE_URL}` are REQUIRED inputs. Phase 0 validates both before any git work. Every PR body MUST include the `## Related Tech Spec` block linking to `juspay/grace#{TECHSPEC_ISSUE_NUMBER}`. There is no path that skips this block. Never fabricate an issue number. Do NOT use `Closes` keywords for the cross-repo reference; use a plain link/reference so the issue remains open until manually resolved.
