...
```json
{
  ...
  "commitHash": "<commit SHA from Phase 1d>",
  "reason": ""
}
```
...
- For SUCCESS connectors: `approved=true`, `status="SUCCESS"`, `prUrl` is the URL captured from `gh pr create` output (or from the existing-PR check in Phase 6a), `reason` is empty.
- For FAILED connectors: `approved=false`, `status="FAILED"`, `prUrl` is still the URL of the do-not-merge PR (Rule 12: every connector gets a PR), `reason` explains the failure.
- `branchName` is always `feat/grace-{connector}-{flow}` (the branch you pushed in Phase 4).
- `commitHash` is the SHA stored in Phase 1d.
