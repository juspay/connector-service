...
## PHASE 0: PRE-SEEDED LINKS CHECK (GooglePay only)

**Run this phase FIRST, before any web discovery, if `{{PAYMENT_METHOD}}` is `GooglePay`.**

1. Read `data/gpay-links.json`.
2. Look up `{{CONNECTOR_NAME}}` as a key (exact match, case-sensitive).
3. **If the connector is found:**
   - Skip Phase 1 (web discovery) entirely.
   - Use the pre-seeded URLs as the verified link set.
   - Proceed directly to **Phase 2** to verify each URL (accessibility + content check).
   - After verification, proceed to Phase 3 to write results.
4. **If the connector is NOT found in `data/gpay-links.json`:**
   - Continue with Phase 1 (normal web discovery).

This ensures that connectors with hard-to-crawl documentation sites (JS SPAs, GitHub raw files, downed servers) still receive correct, pre-researched links rather than incorrect or empty results from blind crawling.
