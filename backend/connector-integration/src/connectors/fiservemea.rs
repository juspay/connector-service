...
macros::create_all_prerequisites!(
    ...
    member_functions: {
        pub fn build_headers<F, FCD, Req, Res>(
            ...
            let client_request_id = fiservemea::generate_client_request_id();
            let timestamp = fiservemea::generate_timestamp();

            let request_body_json = serde_json::to_string(&req)
        ...
        pub fn generate_client_request_id() -> String {
            fiservemea::generate_client_request_id()
        }

        pub fn generate_timestamp() -> i64 {
            fiservemea::generate_timestamp()
        }
        ...
        pub fn generate_message_signature(
            ...
            fiservemea::generate_message_signature(api_key, client_request_id, timestamp, request_body, api_secret)
        }
    }
);
    ...
    for Fiservemea<T>
{
    ...
    fn build_error_response(
        ...
        let attempt_status = match response.response_type {
            fiservemea::FiservemeaResponseType::BadRequest => Some(AttemptStatus::Failure),
            fiservemea::FiservemeaResponseType::Unauthenticated => {
                Some(AttemptStatus::AuthenticationFailed)
            }
            fiservemea::FiservemeaResponseType::Unauthorized => {
                Some(AttemptStatus::AuthorizationFailed)
            }
            fiservemea::FiservemeaResponseType::NotFound => Some(AttemptStatus::Failure),
            fiservemea::FiservemeaResponseType::GatewayDeclined => Some(AttemptStatus::Failure),
            fiservemea::FiservemeaResponseType::EndpointDeclined => Some(AttemptStatus::Failure),
            ...
            fiservemea::FiservemeaResponseType::UnsupportedMediaType => {
        ...
        Ok(ErrorResponse {
            ...
            reason: response.error.decline_reason_code,
            attempt_status,
