...
impl TryFrom<&ConnectorSpecificAuth> for TrustlyAuthType {
    type Error = error_stack::Report<errors::ConnectorError>;
    fn try_from(auth_type: &ConnectorSpecificAuth) -> Result<Self, Self::Error> {
        match auth_type {
            ConnectorSpecificAuth::Trustly {
                username,
                password,
                private_key,
                ..
    ...
    > for TrustlyRefundRequest
{
    ...
    fn try_from(
        ...
        let auth_details = TrustlyAuthType::try_from(&item.router_data.connector_auth_type)?;
        let uuid = uuid::Uuid::new_v4().to_string();
