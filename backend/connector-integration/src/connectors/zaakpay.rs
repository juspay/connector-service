...
use serde::Serialize;
use transformers::{
    self as zaakpay, ZaakPayPaymentsRequest, ZaakPayPaymentsResponse, ZaakPayPaymentsSyncRequest,
    ZaakPayPaymentsSyncResponse, ZaakPayRefundSyncRequest, ZaakPayRefundSyncResponse,
};
...
// Additional ConnectorIntegrationV2 implementations for authentication flows
impl<
        T: PaymentMethodDataTypes
            + std::fmt::Debug
            + std::marker::Sync
...
macros::macro_connector_implementation!(
    ...
    other_functions: {
        ...
        fn get_url(
            ...
            Ok(format!("{}/check.do", self.connector_base_url_payments(req)))
        }
    }
);
...
macros::macro_connector_implementation!(
    ...
    other_functions: {
        ...
        fn get_url(
            ...
            Ok(format!("{}/check.do", self.connector_base_url_refunds(req)))
        }
    }
);
...
// Additional required ConnectorIntegrationV2 implementations
impl<
        T: PaymentMethodDataTypes
            + std::fmt::Debug
            + std::marker::Sync
