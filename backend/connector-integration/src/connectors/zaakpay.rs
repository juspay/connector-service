...
use domain_types::{
    ...
    router_data::{ConnectorAuthType, ErrorResponse, PaymentMethodToken as RouterPaymentMethodToken},
    ...
    > connector_types::IncomingWebhook for ZaakPay<T>
{
    fn verify_webhook_source(
        ...
        // ZaakPay uses checksum verification for webhooks
        // Extract checksum from request headers or body
        let received_checksum = request
            .headers
            .get("x-checksum")
            .or_else(|| request.headers.get("checksum"))
            .ok_or(errors::ConnectorError::WebhookSignatureNotFound)?;

        let secret = String::from_utf8(webhook_secret.secret.to_vec())
            .change_context(errors::ConnectorError::WebhookSourceVerificationFailed)
            .attach_printable("Could not convert secret to UTF-8")?;

        // Verify checksum using ZaakPay's algorithm
    ...
    fn process_payment_webhook(
        ...
        error_stack::Report<domain_types::errors::ConnectorError>,
    > {
        let webhook: transformers::ZaakPayWebhookResponse = request
            .body
...
macros::create_all_prerequisites!(
    ...
    api: [
        ...
        (flow: Void, request_body: ZaakPayVoidRequest, response_body: ZaakPayVoidResponse, router_data: RouterDataV2<Void, PaymentFlowData, PaymentVoidData, PaymentsResponseData>),
        (flow: Capture, request_body: ZaakPayCaptureRequest, response_body: ZaakPayCaptureResponse, router_data: RouterDataV2<Capture, PaymentFlowData, PaymentsCaptureData, PaymentsResponseData>),
        (flow: Refund, request_body: ZaakPayRefundRequest, response_body: ZaakPayRefundResponse, router_data: RouterDataV2<Refund, RefundFlowData, RefundsData, RefundsResponseData>),
        (flow: RSync, request_body: ZaakPayRefundSyncRequest, response_body: ZaakPayRefundSyncResponse, router_data: RouterDataV2<RSync, RefundFlowData, RefundSyncData, RefundsResponseData>),
        (flow: CreateOrder, request_body: ZaakPayCreateOrderRequest, response_body: ZaakPayCreateOrderResponse, router_data: RouterDataV2<CreateOrder, PaymentFlowData, PaymentCreateOrderData, PaymentCreateOrderResponse>),
        (flow: CreateSessionToken, request_body: ZaakPaySessionTokenRequest, response_body: ZaakPaySessionTokenResponse, router_data: RouterDataV2<CreateSessionToken, PaymentFlowData, SessionTokenRequestData, SessionTokenResponseData>),
        (flow: SetupMandate, request_body: ZaakPaySetupMandateRequest, response_body: ZaakPaySetupMandateResponse, router_data: RouterDataV2<SetupMandate, PaymentFlowData, SetupMandateRequestData<T>, PaymentsResponseData>),
...
macros::macro_connector_implementation!(
    ...
    other_functions: {
        fn get_headers(
            ...
            let mut auth_header = get_zaakpay_auth_header(&auth_type)?;
        ...
        fn get_url(
            ...
            Ok(format!("{}/transaction/authorize", self.connector_base_url_payments(req)))
        }
    }
);
...
fn get_zaakpay_auth_header(
    ...
    Ok(vec![(
        ...
        format!("Bearer {}", auth_type.merchant_identifier.peek()).into_masked(),
    )])
}