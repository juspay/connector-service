...
fn construct_billdesk_message<T: PaymentMethodDataTypes + std::fmt::Debug + std::marker::Sync + std::marker::Send + 'static + serde::Serialize>(
        ...
        let (status, response) = match response {
            ...
            BilldeskPaymentsResponse::BilldeskInitNB(response_data) => {
                ...
                let redirection_data = Ok(RedirectForm::Form {
                    endpoint: response_data.rdata.url.clone().unwrap_or_else(|| "".to_string()),
                    method: Method::Post,
                    form_fields: response_data
                        .rdata
                        .parameters
