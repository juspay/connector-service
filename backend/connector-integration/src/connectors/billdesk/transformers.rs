    ...
    > for BilldeskPaymentsRequest
{
    ...
    fn try_from(
        ...
        let customer_id = item.router_data.resource_common_data.get_customer_id()?.get_string_repr().to_string();
        ...
        let msg = match item.router_data.request.payment_method_type {
            Some(common_enums::PaymentMethodType::UpiCollect) => {
                build_upi_message(&item, &customer_id, &transaction_id, &amount.to_string())?
            }
