...
mod tests {
    ...
    mod url_tests {
        ...
        fn test_authorize_url_generation() {
            ...
            assert!(url.contains("order/ref_123456789"));
            assert!(url.contains("transaction/ref_123456789"));
        }
        ...
        fn test_capture_url_generation() {
            ...
            let url = request.as_ref().unwrap().url.clone();
            assert!(url.contains("merchant/"));
            assert!(url.contains("order/ref_capture"));
            assert!(url.contains("transaction/TXN_CAPTURE_123"));
        }
        ...
        fn test_void_url_generation() {
            ...
            let url = request.as_ref().unwrap().url.clone();
            assert!(url.contains("merchant/"));
            assert!(url.contains("order/ref_void"));
