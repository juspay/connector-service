...
use domain_types::{
    ...
    connector_types::{
        PaymentFlowData, PaymentsAuthorizeData, PaymentsResponseData, PaymentsSyncData, ResponseId, Status,
    },
    ...
    > for PayuPaymentRequest
{
    ...
    fn try_from(
        ...
        let mut request = Self {
            ...
            s2s_client_ip: router_data.request.get_ip_address_as_optional().map(|ip_address| ip_address.expose().into()).ok_or_else(|| {
...
>(
    ...
    match &request.payment_method_data {
        ...
        _ => Err(ConnectorError::NotSupported {
            message:
                "Payment method not supported by PayU. Only UPI payments are supported"
                    .to_string(),
            connector: "PayU",
        }),
    }
    ...
    fn try_from(
        ...
        match (response.status, response.transaction_details) {
            (Some(1), Some(transaction_details)) => {
                ...
                let connector_transaction_id = txn_detail
                ...
                match (txn_detail, connector_transaction_id) {
                    (Some(txn_detail), Some(connector_transaction_id)) => {
                        ...
                        let attempt_status = 
                            map_payu_sync_status(&txn_detail.status, txn_detail);
