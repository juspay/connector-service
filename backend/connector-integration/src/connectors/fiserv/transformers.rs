...
use hyperswitch_common_utils::{
    pii, 
    types::{FloatMajorUnit, AmountConvertor, FloatMajorUnitForConnector},
};
...
use hyperswitch_interfaces::{errors::ConnectorError, consts::{NO_ERROR_CODE, NO_ERROR_MESSAGE}};
...
use domain_types::{
    connector_flow::{Authorize as AuthorizeFlow, Capture as CaptureFlow, PSync as PSyncFlow, Refund as RefundFlow, RSync as RSyncFlow, Void as VoidFlow}, 
    connector_types::{
        PaymentFlowData, PaymentsAuthorizeData, PaymentsCaptureData, 
        PaymentsResponseData, PaymentsSyncData, ResponseId, RefundFlowData, RefundsData, RefundsResponseData, RefundSyncData, PaymentVoidData,
    },
};
...
fn get_card_expiry_year_4_digit_placeholder(year_yy: &Secret<String>) -> Result<Secret<String>, error_stack::Report<ConnectorError>> {
...
impl From<FiservPaymentStatus> for enums::RefundStatus {
    fn from(item: FiservPaymentStatus) -> Self {
        match item {
            FiservPaymentStatus::Captured | FiservPaymentStatus::Succeeded | FiservPaymentStatus::Authorized => Self::Success,
            FiservPaymentStatus::Declined | FiservPaymentStatus::Failed => Self::Failure,
...
impl TryFrom<&Option<pii::SecretSerdeValue>> for FiservSessionObject {
    ...
    fn try_from(meta_data: &Option<pii::SecretSerdeValue>) -> Result<Self, Self::Error> {
        let secret_value_str = meta_data
            .as_ref()
            .ok_or_else(|| report!(ConnectorError::MissingRequiredField {field_name: "connector_meta_data (FiservSessionObject)"}))
...
impl TryFrom<FiservRouterData<RouterDataV2<AuthorizeFlow, PaymentFlowData, PaymentsAuthorizeData, PaymentsResponseData>>> for FiservPaymentsRequest {
    type Error = error_stack::Report<ConnectorError>;
    fn try_from(
        item: FiservRouterData<RouterDataV2<AuthorizeFlow, PaymentFlowData, PaymentsAuthorizeData, PaymentsResponseData>>,
        ...
        let session_meta_value = router_data.resource_common_data.connector_meta_data
            .as_ref()
            .ok_or_else(|| report!(ConnectorError::MissingRequiredField { field_name: "connector_meta_data for FiservSessionObject" }))?
            .peek();
...
impl TryFrom<FiservRouterData<RouterDataV2<CaptureFlow, PaymentFlowData, PaymentsCaptureData, PaymentsResponseData>>> for FiservCaptureRequest {
    type Error = error_stack::Report<ConnectorError>;
    fn try_from(item: FiservRouterData<RouterDataV2<CaptureFlow, PaymentFlowData, PaymentsCaptureData, PaymentsResponseData>>) -> Result<Self, Self::Error> {
        let router_data = item.router_data;
        ...
        let session_str = if let Some(meta) = router_data.resource_common_data.connector_meta_data.as_ref() {
            ...
            return Err(report!(ConnectorError::MissingRequiredField { 
                field_name: "connector_metadata or connector_meta_data for FiservSessionObject in Capture" 
            }));
        };
        ...
        let amount_major = converter.convert(router_data.request.minor_amount_to_capture, router_data.request.currency)
            .change_context(ConnectorError::RequestEncodingFailed)?;
...
impl TryFrom<FiservRouterData<RouterDataV2<PSyncFlow, PaymentFlowData, PaymentsSyncData, PaymentsResponseData>>> for FiservSyncRequest {
    type Error = error_stack::Report<ConnectorError>;
    fn try_from(item: FiservRouterData<RouterDataV2<PSyncFlow, PaymentFlowData, PaymentsSyncData, PaymentsResponseData>>) -> Result<Self, Self::Error> {
        let router_data = &item.router_data;
        let auth: FiservAuthType = FiservAuthType::try_from(&router_data.connector_auth_type)?;
        Ok(Self {
...
impl TryFrom<FiservRouterData<RouterDataV2<VoidFlow, PaymentFlowData, PaymentVoidData, PaymentsResponseData>>> for FiservVoidRequest {
    type Error = error_stack::Report<ConnectorError>;
    fn try_from(item: FiservRouterData<RouterDataV2<VoidFlow, PaymentFlowData, PaymentVoidData, PaymentsResponseData>>) -> Result<Self, Self::Error> {
        let router_data = &item.router_data;
        let auth: FiservAuthType = FiservAuthType::try_from(&router_data.connector_auth_type)?;
        ...
        let session_meta_value = router_data.resource_common_data.connector_meta_data
            .as_ref()
            .ok_or_else(|| report!(ConnectorError::MissingRequiredField { 
...
impl TryFrom<FiservRouterData<RouterDataV2<RefundFlow, RefundFlowData, RefundsData, RefundsResponseData>>> for FiservRefundRequest {
    type Error = error_stack::Report<ConnectorError>;
    fn try_from(item: FiservRouterData<RouterDataV2<RefundFlow, RefundFlowData, RefundsData, RefundsResponseData>>) -> Result<Self, Self::Error> {
        let router_data = &item.router_data;
        let auth: FiservAuthType = FiservAuthType::try_from(&router_data.connector_auth_type)?;
        ...
        let session_str = if let Some(connector_meta) = router_data.request.connector_metadata.as_ref() {
            ...
            match connector_meta {
                ...
                _ => return Err(report!(ConnectorError::InvalidConnectorConfig {
                    config: "connector_metadata was not a JSON string for FiservSessionObject in Refund",
                })),
            }
        } else {
            // No metadata available
            return Err(report!(ConnectorError::MissingRequiredField { 
                field_name: "connector_metadata or connector_meta_data for FiservSessionObject in Refund" 
            }));
        };
        ...
        let converter = FloatMajorUnitForConnector;
        let amount_major = converter.convert(router_data.request.minor_refund_amount, router_data.request.currency)
            .change_context(ConnectorError::RequestEncodingFailed)?;
...
impl TryFrom<FiservRouterData<RouterDataV2<RSyncFlow, RefundFlowData, RefundSyncData, RefundsResponseData>>> for FiservRefundSyncRequest {
    type Error = error_stack::Report<ConnectorError>;
    fn try_from(item: FiservRouterData<RouterDataV2<RSyncFlow, RefundFlowData, RefundSyncData, RefundsResponseData>>) -> Result<Self, Self::Error> {
        let router_data = &item.router_data;
        let auth: FiservAuthType = FiservAuthType::try_from(&router_data.connector_auth_type)?;
        Ok(Self {
            merchant_details: MerchantDetails {
        ...
        let response_payload = PaymentsResponseData::TransactionResponse {
            resource_id: ResponseId::ConnectorTransactionId(
                gateway_resp.gateway_transaction_id.clone().unwrap_or_else(|| 
                    gateway_resp.transaction_processing_details.transaction_id.clone()),
            ),
    ...
    for RouterDataV2<F, PaymentFlowData, PaymentVoidData, PaymentsResponseData>
{
    ...
    fn try_from(
        item: ResponseRouterData<FiservVoidResponse, Self>,
    ) -> Result<Self, Self::Error> {
        let ResponseRouterData {
            response,
            router_data,
    ...
    for RouterDataV2<F, PaymentFlowData, PaymentsSyncData, PaymentsResponseData>
{
    ...
    fn try_from(
        item: ResponseRouterData<FiservSyncResponse, Self>,
    ) -> Result<Self, Self::Error> {
        let ResponseRouterData {
            response,
            router_data,
        ...
        let fiserv_payment_response = response.sync_responses.first()
    ...
    for RouterDataV2<F, RefundFlowData, RefundsData, RefundsResponseData>
{
    ...
    fn try_from(
        item: ResponseRouterData<FiservRefundResponse, Self>,
    ) -> Result<Self, Self::Error> {
        let ResponseRouterData {
            response,
            router_data,
        ...
        let response_payload = RefundsResponseData {
            connector_refund_id: gateway_resp.gateway_transaction_id.clone().unwrap_or_else(|| 
                gateway_resp.transaction_processing_details.transaction_id.clone()),
            refund_status,
        };
    ...
    for RouterDataV2<F, PaymentFlowData, Req, Res>
{
    ...
    fn try_from(
        item: ResponseRouterData<FiservErrorResponse, Self>,
    ) -> Result<Self, Self::Error> {
        let ResponseRouterData {
            response,
            router_data,
        ...
        let error_details = response.error.as_ref().or(response.details.as_ref())
            .and_then(|e| e.first());
        
        let message = error_details.map_or(NO_ERROR_MESSAGE.to_string(), |e| e.message.clone());
        let code = error_details.and_then(|e| e.code.clone()).unwrap_or_else(|| NO_ERROR_CODE.to_string());
