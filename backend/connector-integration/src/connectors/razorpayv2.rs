    ...
    for RazorpayV2
{
    ...
    fn get_request_body(
        ...
        match version {
            "v1" => {
                info!("V1 was selected for razorpay");
                let connector_req =
                    razorpayv2::RazorpayWebCollectRequest::try_from(&connector_router_data)?;
                Ok(Some(RequestContent::Json(Box::new(connector_req))))
            }
            _ => {
                // Default to v2 request format
                let connector_req =
    ...
    for RazorpayV2
{
    ...
    fn get_url(
        ...
        tracing::info!("RazorpayV2 PSync URL: {}", url);
        Ok(url)
    }
    ...
    fn handle_response_v2(
        ...
        errors::ConnectorError,
    > {
        // For PSync, Razorpay returns the payment object directly, not wrapped in our custom structure
        let response: razorpayv2::RazorpayV2PaymentsResponse = res
            .response
            .parse_struct("RazorpayV2PaymentsResponse")
            .change_context(errors::ConnectorError::ResponseDeserializationFailed)?;
