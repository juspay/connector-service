...
use domain_types::{
    ...
    connector_types::{
        ...
        PaymentsResponseData, RefundFlowData, RefundsData,
...
impl<F, T> TryFrom<PproRouterData<RouterDataV2<F, PaymentFlowData, PaymentsAuthorizeData<T>, PaymentsResponseData>, T>> for PproPaymentsRequest
where
    T: Clone + PaymentMethodDataTypes + std::fmt::Debug + Sync + Send + 'static + Serialize,
{
    type Error = error_stack::Report<errors::ConnectorError>;
    fn try_from(
        item: PproRouterData<RouterDataV2<F, PaymentFlowData, PaymentsAuthorizeData<T>, PaymentsResponseData>, T>,
        ...
        let consumer = router_data.resource_common_data.get_billing_address().ok().map(|billing| {
        ...
        Ok(Self {
            ...
            amount,
            consumer,
            authentication_settings: if authentication_settings.is_empty() { None } else { Some(authentication_settings) },
        })
    }
}
...
impl<T> TryFrom<PproRouterData<RouterDataV2<Capture, PaymentFlowData, PaymentsCaptureData, PaymentsResponseData>, T>> for PproCaptureRequest
where
    T: PaymentMethodDataTypes + std::fmt::Debug + Sync + Send + 'static + Serialize,
{
    type Error = error_stack::Report<errors::ConnectorError>;
    fn try_from(item: PproRouterData<RouterDataV2<Capture, PaymentFlowData, PaymentsCaptureData, PaymentsResponseData>, T>) -> Result<Self, Self::Error> {
        Ok(Self {
            amount: Amount {
...
impl<T> TryFrom<PproRouterData<RouterDataV2<Void, PaymentFlowData, PaymentVoidData, PaymentsResponseData>, T>> for PproVoidRequest
where
    T: PaymentMethodDataTypes + std::fmt::Debug + Sync + Send + 'static + Serialize,
{
    type Error = error_stack::Report<errors::ConnectorError>;
    fn try_from(_item: PproRouterData<RouterDataV2<Void, PaymentFlowData, PaymentVoidData, PaymentsResponseData>, T>) -> Result<Self, Self::Error> {
        Ok(Self {})
    }
}
...
impl<T> TryFrom<PproRouterData<RouterDataV2<Refund, RefundFlowData, RefundsData, RefundsResponseData>, T>> for PproRefundRequest
where
    T: PaymentMethodDataTypes + std::fmt::Debug + Sync + Send + 'static + Serialize,
{
    type Error = error_stack::Report<errors::ConnectorError>;
    fn try_from(item: PproRouterData<RouterDataV2<Refund, RefundFlowData, RefundsData, RefundsResponseData>, T>) -> Result<Self, Self::Error> {
        Ok(Self {
            amount: Amount {
                currency: item.router_data.request.currency.to_string(),
...
impl<F, Req> TryFrom<ResponseRouterData<PproPaymentsResponse, RouterDataV2<F, PaymentFlowData, Req, PaymentsResponseData>>>
    ...
    fn try_from(
        ...
        if status == common_enums::AttemptStatus::Failure {
            if let Some(failure) = &item.response.failure {
                 let fallback_msg = failure.failure_code.clone().unwrap_or_else(|| "UNKNOWN".to_string());
                 ...
                 error_response = Some(ErrorResponse {
                     ...
                     code: failure.failure_code.clone().unwrap_or_else(|| "UNKNOWN".to_string()),
                     message,
        ...
        if status == common_enums::AttemptStatus::AuthenticationPending {
             if let Some(auth_methods) = item.response.authentication_methods.as_ref() {
                  ...
                  if let Some(meta_data) = item.router_data.resource_common_data.connector_meta_data.clone() {
                  ...
                  let priorities = if is_sdk_flow {
                      vec!["APP_INTENT", "SCAN_CODE", "APP_NOTIFICATION", "REDIRECT", "MULTI_FACTOR"]
                  } else {
                      vec!["REDIRECT", "APP_NOTIFICATION", "MULTI_FACTOR", "SCAN_CODE", "APP_INTENT"]
                  };
...
impl<F, Req, T> TryFrom<ResponseRouterData<PproPaymentsResponse, RouterDataV2<F, Req, T, RefundsResponseData>>>
    for RouterDataV2<F, Req, T, RefundsResponseData>
{
    ...
    fn try_from(
        ...
        let response = if refund_status == common_enums::RefundStatus::Failure {
            let failure_code = item.response.failure.as_ref().and_then(|f| f.failure_code.clone());
            let failure_message = item.response.failure.as_ref().map_or("".to_string(), |f| f.failure_message.clone());
            let failure_type = item.response.failure.as_ref().map_or("".to_string(), |f| f.failure_type.clone());
...
pub fn get_error_message(failure_code: Option<&str>) -> String {
    match failure_code {
        Some("ACCOUNT_NOT_SUPPORTED") => "The consumer's account or card not supported.".to_string(),
        Some("AMOUNT_NOT_SUPPORTED") => "Amount value is invalid or outside supported range.".to_string(),
        Some("AUTHENTICATION_FAILED") => "The consumer failed authentication.".to_string(),
        Some("CANCELED_BY_CONSUMER") => "The consumer intentionally abandoned the authentication flow.".to_string(),
        Some("COUNTRY_NOT_SUPPORTED") => "The country value is not supported by the provider.".to_string(),
        Some("DUPLICATE_DETECTED") => "Provider detected a very similar request recently with the same details and amount.".to_string(),
        ...
        Some("INCORRECT_HOLDER_NAME") => "The card or account holderName could not be validated by the issuer.".to_string(),
        Some("INCORRECT_ISSUER") => "The card number/issuer is not within a card number range supported by the provider.".to_string(),
        Some("INCORRECT_NUMBER") => "The card or account number cannot be validated by the issuer.".to_string(),
        Some("INSUFFICIENT_FUNDS") => "Insufficient funds in the account.".to_string(),
        Some("IS_EXCEEDING_ACCOUNT_LIMIT") => "The amount value exceeds the limit on the consumer's account.".to_string(),
        Some("IS_GENERIC_DECLINE") => "Generic decline by the issuer.".to_string(),
        Some("IS_HIGH_VELOCITY") => "The consumer has exceeded the number of attempts allowed by the issuer.".to_string(),
        Some("IS_NOT_PERMITTED") => "Not permitted by the issuer.".to_string(),
        Some("SUS_FRAUD") => "The payment was declined because the issuer or provider suspects it was fraudulent.".to_string(),
        Some("SUS_LOST") => "The payment was declined because the consumer's card was reported lost.".to_string(),
        Some("SUS_STOLEN") => "The payment was declined because the consumer's card was reported stolen.".to_string(),
        Some("AGREEMENT_INACTIVE") => "Payment Agreement not yet active or failed during setup.".to_string(),
        Some("AGREEMENT_REVOKED") => "Payment Agreement revoked by the consumer or merchant.".to_string(),
        Some("BLOCKED_ACCOUNT") => "Account or card blocked by PPRO due to suspected fraud or abuse.".to_string(),
        Some("BLOCKED_BIN") => "BIN (first 6–8 digits) blocked by PPRO due to high risk or chargebacks.".to_string(),
        Some("BLOCKED_EMAIL") => "Consumer email blocked by PPRO due to suspected fraud or abuse.".to_string(),
        Some("BLOCKED_IP") => "Consumer IP blocked by PPRO due to suspected fraud or abuse.".to_string(),
        Some("EXCEEDS_AUTHORIZED_AMOUNT") => "Capture or void amount exceeds the remaining authorized amount.".to_string(),
        Some("EXCEEDS_CAPTURED_AMOUNT") => "Refund amount exceeds the captured funds.".to_string(),
        Some("EXCEEDS_REFUND_LIMIT") => "Refund would reduce the account balance below minimum.".to_string(),
        Some("EXCEEDS_REFUNDABLE_AMOUNT") => "Refund attempted before capture or after full refund.".to_string(),
        Some("HIGH_ACCOUNT_VELOCITY") => "Too many attempts from the same account or card.".to_string(),
        Some("HIGH_BIN_VELOCITY") => "Too many attempts from the same BIN range.".to_string(),
        Some("HIGH_EMAIL_VELOCITY") => "Too many attempts using the same email address.".to_string(),
        ...
        Some("INVALID_AUTHENTICATION_SETTINGS") => "Invalid or missing `authenticationSettings` for the payment method.".to_string(),
        Some("INVALID_COUNTRY") => "Unsupported or invalid country for this payment method.".to_string(),
        Some("INVALID_CURRENCY") => "Unsupported or invalid currency for this payment method.".to_string(),
        ...
        Some("INVALID_TAX_ID") => "Invalid `consumer.taxIdentification` for this payment method".to_string(),
        Some("NO_AUTHORIZED_AMOUNT") => "No successful Authorization found; capture/void not possible.".to_string(),
        Some("PAYMENT_METHOD_TIMEOUT") => "Authorization not completed within timeout limit set by PPRO.".to_string(),
        Some("UNRESPONSIVE_ISSUER") => "The issuer cannot be contacted/is unavailable.".to_string(),
        Some("UNRESPONSIVE_PROVIDER") => "The provider cannot be contacted/is unavailable.".to_string(),
        Some("PROVIDER_PROCESSING_ERROR") => "The provider could not process the request we sent to them.".to_string(),
        Some("INTERNAL_SERVICE_UNAVAILABLE") => "A PPRO internal service is temporarily down during request processing.".to_string(),
        Some("INTERNAL_PROCESSING_ERROR") => "A PPRO internal service encountered an unexpected error while handling the request.".to_string(),
