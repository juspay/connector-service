...
pub fn check_currency(currency: common_enums::Currency) -> Result<common_enums::Currency, ConnectorError> {
...
pub struct HelcimPaymentsRequest<T: PaymentMethodDataTypes + std::fmt::Debug + Sync + Send + 'static + Serialize> {
...
#[derive(Debug, Serialize)]
#[serde(rename_all = "camelCase")]
pub struct HelcimCard<T: PaymentMethodDataTypes + std::fmt::Debug + Sync + Send + 'static + Serialize> {
...
impl<T: PaymentMethodDataTypes + std::fmt::Debug + Sync + Send + 'static + Serialize>
    TryFrom<
        HelcimRouterData<RouterDataV2<Authorize, PaymentFlowData, PaymentsAuthorizeData<T>, PaymentsResponseData>, T>,
    ...
    fn try_from(
        ...
        let card_data = match &item.router_data.request.payment_method_data {
            PaymentMethodData::Card(card) => HelcimCard {
                card_expiry: card.get_card_expiry_month_year_2_digit_with_delimiter("".to_string())?,
...
{
    type Error = error_stack::Report<ConnectorError>;
    fn try_from(item: ResponseRouterData<HelcimPaymentsResponse, Self>) -> Result<Self, Self::Error> {
    ...
    fn try_from(item: ResponseRouterData<HelcimPaymentsResponse, Self>) -> Result<Self, Self::Error> {
        match item.router_data.request.sync_type {
            SyncRequestType::SinglePaymentSync => {
                ...
                Ok(Self {
                    response: Ok(PaymentsResponseData::TransactionResponse {
                        resource_id: ResponseId::ConnectorTransactionId(item.response.transaction_id.to_string()),
                        redirection_data: None,
            ...
            SyncRequestType::MultipleCaptureSync => {
                Err(ConnectorError::NotImplemented("manual multiple capture sync".to_string()).into())
...
impl<T: PaymentMethodDataTypes + std::fmt::Debug + Sync + Send + 'static + Serialize>
    TryFrom<HelcimRouterData<RouterDataV2<Capture, PaymentFlowData, PaymentsCaptureData, PaymentsResponseData>, T>>
...
impl<T: PaymentMethodDataTypes + std::fmt::Debug + Sync + Send + 'static + Serialize>
    TryFrom<HelcimRouterData<RouterDataV2<Void, PaymentFlowData, PaymentVoidData, PaymentsResponseData>, T>>
...
#[derive(Debug, Serialize)]
#[serde(rename_all = "camelCase")]
pub struct HelcimSetupMandateRequest<T: PaymentMethodDataTypes + std::fmt::Debug + Sync + Send + 'static + Serialize> {
    amount: FloatMajorUnit,
    currency: common_enums::Currency,
    ip_address: Secret<String, pii::IpAddress>,
    card_data: HelcimCard<T>,
    ...
    for HelcimRefundRequest
{
    type Error = error_stack::Report<ConnectorError>;
    fn try_from(
        item: HelcimRouterData<RouterDataV2<F, RefundFlowData, RefundsData, RefundsResponseData>, T>,
    ) -> Result<Self, Self::Error> {
