...
use domain_types::{
    ...
    connector_types::{
        ...
        PaymentsAuthorizeData, PaymentsCaptureData, PaymentsIncrementalAuthorizationData, PaymentsPostAuthenticateData,
...
use transformers::{
    self as truelayer, TruelayerAccessTokenRequestData, TruelayerAccessTokenResponseData, TruelayerPSyncResponseData,
    TruelayerPaymentsRequestData, TruelayerPaymentsResponseData, TruelayerRefundRequest, TruelayerRefundResponse,
    TruelayerRsyncResponse, TruelayerVoidResponseData,
};
...
impl<T: PaymentMethodDataTypes + Debug + Sync + Send + 'static + Serialize> connector_types::RefundV2 for Truelayer<T> {}
...
impl<T: PaymentMethodDataTypes + Debug + Sync + Send + 'static + Serialize> SourceVerification for Truelayer<T> {}
impl<T: PaymentMethodDataTypes + Debug + Sync + Send + 'static + Serialize> BodyDecoding for Truelayer<T> {}
...
macros::create_all_prerequisites!(
    ...
    member_functions: {
            ...
            Self: ConnectorIntegrationV2<F, FlowData, Req, Res>,
        {
            ...
            let tl_signature = self.clone().generate_tl_signature(http_method.to_string(), path.as_str(), &headers, body_json_str, private_key, kid.as_str())?;
...
impl<T: PaymentMethodDataTypes + Debug + Sync + Send + 'static + Serialize> ConnectorCommon for Truelayer<T> {
...
impl<T: PaymentMethodDataTypes + Debug + Sync + Send + 'static + Serialize>
    ConnectorIntegrationV2<Void, PaymentFlowData, PaymentVoidData, PaymentsResponseData> for Truelayer<T>
{
}

impl<T: PaymentMethodDataTypes + Debug + Sync + Send + 'static + Serialize>
    ConnectorIntegrationV2<Capture, PaymentFlowData, PaymentsCaptureData, PaymentsResponseData> for Truelayer<T>
{
}
...
impl<T: PaymentMethodDataTypes + Debug + Sync + Send + 'static + Serialize>
    ConnectorIntegrationV2<SubmitEvidence, DisputeFlowData, SubmitEvidenceData, DisputeResponseData> for Truelayer<T>
{
}

impl<T: PaymentMethodDataTypes + Debug + Sync + Send + 'static + Serialize>
    ConnectorIntegrationV2<DefendDispute, DisputeFlowData, DisputeDefendData, DisputeResponseData> for Truelayer<T>
{
}

impl<T: PaymentMethodDataTypes + Debug + Sync + Send + 'static + Serialize>
    ConnectorIntegrationV2<Accept, DisputeFlowData, AcceptDisputeData, DisputeResponseData> for Truelayer<T>
{
}
