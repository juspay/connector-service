...
pub struct Card<T: PaymentMethodDataTypes + std::fmt::Debug + std::marker::Sync + std::marker::Send + 'static + Serialize> {
...
pub struct DlocalRouterData<T, U> {
    pub amount: MinorUnit,
    pub router_data: T,
    pub payment_method_data: std::marker::PhantomData<U>,
}

impl<T, U> TryFrom<(MinorUnit, T)> for DlocalRouterData<T, U> {
...
impl TryFrom<DlocalRouterData<RouterDataV2<PSync, PaymentFlowData, PaymentsSyncData, PaymentsResponseData>, ()>> for DlocalPaymentsSyncRequest {
    type Error = error_stack::Report<errors::ConnectorError>;
    fn try_from(item: DlocalRouterData<RouterDataV2<PSync, PaymentFlowData, PaymentsSyncData, PaymentsResponseData>, ()>) -> Result<Self, Self::Error> {
        Ok(Self {
            authz_id: item.router_data.request.get_connector_transaction_id()?,
        })
    }
}
...
impl<T: PaymentMethodDataTypes + std::fmt::Debug + std::marker::Sync + std::marker::Send + 'static + Serialize> TryFrom<DlocalRouterData<RouterDataV2<Void, PaymentFlowData, PaymentVoidData, PaymentsResponseData>, T>> for DlocalPaymentsCancelRequest {
    type Error = error_stack::Report<errors::ConnectorError>;
    fn try_from(item: DlocalRouterData<RouterDataV2<Void, PaymentFlowData, PaymentVoidData, PaymentsResponseData>, T>) -> Result<Self, Self::Error> {
        Ok(Self {
            cancel_id: item.router_data.request.connector_transaction_id.clone(),
        })
    }
}
    ...
    > for DlocalPaymentsCaptureRequest
{
    ...
    fn try_from(
        ...
        Ok(Self {
            authorization_id: item.router_data.request.get_connector_transaction_id()?,
...
impl<
    ...
    >
    TryFrom<
        DlocalRouterData<RouterDataV2<F, RefundFlowData, RefundsData, RefundsResponseData>, T>,
