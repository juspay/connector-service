        ...
        match item.router_data.resource_common_data.payment_method {
            common_enums::PaymentMethod::Upi => Ok(Self {
                ...
                transaction: TpslTransactionPayload {
                    ...
                    identifier: transaction_id.clone(),
                    date_time: OffsetDateTime::now_utc().to_string(),
                    token: auth.security_token.ok_or(ConnectorError::FailedToObtainAuthType)?.clone(),
                    security_token: auth.security_token.ok_or(ConnectorError::FailedToObtainAuthType)?.clone(),
