        ...
        match item.router_data.resource_common_data.payment_method {
            common_enums::PaymentMethod::Upi => Ok(Self {
                ...
                transaction: TpslTransactionPayload {
                    ...
                    identifier: item.router_data.resource_common_data.connector_request_reference_id.clone(),
                    date_time: chrono::Utc::now().to_rfc3339(),
                    token: auth.security_token.ok_or(ConnectorError::FailedToObtainAuthType)?,
                    security_token: auth.security_token.ok_or(ConnectorError::FailedToObtainAuthType)?,
    ...
    fn try_from(
        ...
        Ok(Self {
            merchant: TpslMerchantDataType {
                identifier: auth.merchant_code.ok_or(ConnectorError::FailedToObtainAuthType)?.expose(),
            },
