...
impl<T: PaymentMethodDataTypes + Debug + Sync + Send + 'static + Serialize>
    connector_types::PaymentVoidPostCaptureV2<T> for Dlocal<T>
{
}
    ...
    connector_types::ValidationTrait for Dlocal<T>
{
    fn should_do_order_create(&self) -> bool {
        false // Dlocal doesn't require separate order creation
    }
}
    ...
    ($flow:ty, $common_data:ty, $req:ty, $resp:ty) => {
        impl<
                T: PaymentMethodDataTypes
                    ...
                    + 'static
                    + Serialize,
            > interfaces::verification::SourceVerification<$flow, $common_data, $req, $resp> for Dlocal<T>
        {
            fn get_secrets(
                &self,
