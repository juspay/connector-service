...
use domain_types::{
    connector_flow::{
        ...
        Refund, RepeatPayment, SetupMandate, SubmitEvidence, Void, PreAuthenticate, Authenticate, PostAuthenticate, CreateAccessToken, CreateConnectorCustomer, PaymentMethodToken,
    },
    connector_types::{
...
use serde::Serialize;
use transformers::{
    self as tpsl, TpslPaymentsRequest, TpslPaymentsResponse, TpslPaymentsSyncRequest,
...
// Create all prerequisites using the mandatory UCS v2 macro framework
macros::create_all_prerequisites!(
    ...
    member_functions: {
        ...
        pub fn get_api_tag(&self, flow: &str) -> &'static str {
            match flow {
                "Authorize" => constants::api_tags::AUTH_CAPTURE,
                "PSync" => constants::api_tags::SI_TRANSACTION,
                "RSync" => constants::api_tags::REFUND_ARN_SYNC,
                _ => constants::api_tags::TRANSACTION,
            }
        }
    }
);
    ...
    other_functions: {
        fn get_headers(
            ...
            if let Some(merchant_code) = auth_type.merchant_code {
                header.push((headers::AUTHORIZATION.to_string(), merchant_code.into_masked()));
            }
        ...
        fn get_url(
            ...
            let merchant_code = tpsl::TpslAuth::try_from(&req.connector_auth_type)?
                .merchant_code
            ...
            Ok(format!(
                "{}{}{}",
                base_url,
                constants::endpoints::MERCHANT2_PG,
                format!("/{}", merchant_code)
            ))
        }
    }
);
