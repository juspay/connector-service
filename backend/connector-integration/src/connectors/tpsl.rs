...
use common_utils::{errors::CustomResult, ext_traits::ByteSliceExt, types::StringMinorUnit, crypto};
use domain_types::{
    connector_flow::{
    ...
    other_functions: {
        fn get_headers(
            ...
            if let Some(merchant_code) = auth_type.merchant_code {
                header.push((headers::AUTHORIZATION.to_string(), format!("Bearer {}", merchant_code.peek()).into_masked()));
            }
        ...
        fn get_url(
            ...
            Ok(format!("{}/PaymentGateway/merchant2.pg/{}", base_url, "merchant_code"))
        }
    }
);
...
macros::macro_connector_implementation!(
    ...
    other_functions: {
        ...
        fn get_url(
            ...
            let base_url = self.connector_base_url_payments(req);
            Ok(format!("{}/PaymentGateway/services/SITransaction", base_url))
        }
    }
);
    ...
    > ConnectorCommon for TPSL<T>
{
    ...
    fn build_error_response(
        ...
        let response: tpsl::TpslErrorResponseWrapper = res
            .response
        ...
        Ok(ErrorResponse::from(response))
    }
}
