    ...
    fn try_from(pm_data: &PaymentMethodData<T>) -> Result<Self, Self::Error> {
        match pm_data {
            ...
            PaymentMethodData::Wallet(wallet_data) => {
                let nmi_wallet_data = create_wallet_payment_data::<T>(wallet_data)?;
                Ok(Self::Wallet(nmi_wallet_data))
            }
...
fn create_wallet_payment_data<T: PaymentMethodDataTypes>(
    wallet_data: &WalletData,
) -> Result<WalletPaymentData, error_stack::Report<errors::ConnectorError>> {
    match wallet_data {
        WalletData::ApplePay(apple_pay_data) => {
            let payment_token = extract_applepay_token(apple_pay_data)?;
            Ok(WalletPaymentData { payment_token })
        }
        WalletData::GooglePay(google_pay_data) => {
            let payment_token = extract_googlepay_token(google_pay_data)?;
            Ok(WalletPaymentData { payment_token })
        }
        _ => Err(error_stack::report!(
...
/// Extract payment token from Apple Pay wallet data
/// NMI expects the payment_token field to contain a token generated by their Collect.js SDK
/// For direct API integration, we use the transaction_identifier as a reference
fn extract_applepay_token(
    apple_pay_data: &ApplePayWalletData,
) -> Result<Secret<String>, error_stack::Report<errors::ConnectorError>> {
    Ok(Secret::new(apple_pay_data.transaction_identifier.clone()))
}

/// Extract payment token from Google Pay wallet data
/// NMI expects the payment_token field to contain a token generated by their Collect.js SDK
/// For direct API integration, we use the token from tokenization_data
fn extract_googlepay_token(
    _google_pay_data: &GooglePayWalletData,
) -> Result<Secret<String>, error_stack::Report<errors::ConnectorError>> {
    Err(error_stack::report!(
        errors::ConnectorError::NotImplemented(
            "Google Pay direct API integration not yet supported for NMI".to_string()
        )
    ))
}
