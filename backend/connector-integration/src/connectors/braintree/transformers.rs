...
pub mod constants {
    ...
    pub const TOKENIZE_US_BANK_ACCOUNT: &str = "mutation TokenizeUsBankAccount($input: TokenizeUsBankAccountInput!) { tokenizeUsBankAccount(input: $input) { paymentMethod { id usage createdAt details { ... on UsBankAccountDetails { accountholderName accountType bankName last4 routingNumber verified achMandate { acceptedAt acceptanceText } } } } } }";
    pub const VAULT_US_BANK_ACCOUNT: &str = "mutation VaultUsBankAccount($input: VaultUsBankAccountInput!) { vaultUsBankAccount(input: $input) { paymentMethod { id legacyId details { ... on UsBankAccountDetails { accountholderName accountType bankName last4 routingNumber verified achMandate { acceptedAt acceptanceText } } } } verification { id status } } }";
    pub const VERIFY_US_BANK_ACCOUNT: &str = "mutation VerifyUsBankAccount($input: VerifyUsBankAccountInput!) { verifyUsBankAccount(input: $input) { verification { id legacyId status merchantAccountId createdAt gatewayRejectionReason paymentMethod { id } processorResponse { legacyCode message } } } }";
    pub const CHARGE_US_BANK_ACCOUNT: &str = "mutation ChargeUsBankAccount($input: ChargeUsBankAccountInput!) { chargeUsBankAccount(input: $input) { transaction { id amount { value } status paymentMethodSnapshot { ... on UsBankAccountDetails { accountholderName accountType verified } } } } }";
    pub const AUTHORIZE_US_BANK_ACCOUNT: &str = "mutation AuthorizeUsBankAccount($input: AuthorizeUsBankAccountInput!) { authorizeUsBankAccount(input: $input) { transaction { id amount { value } status paymentMethodSnapshot { ... on UsBankAccountDetails { accountholderName accountType verified } } } } }";
}
    ...
    for RouterDataV2<F, PaymentFlowData, PaymentsAuthorizeData<T>, PaymentsResponseData>
{
    ...
    fn try_from(
        ...
        match item.response {
            ...
            BraintreeAuthResponse::AchBankDebitAuthResponse(ach_response) => {
                let transaction_data = &ach_response.authorize_us_bank_account.transaction;
                let status = enums::AttemptStatus::from(transaction_data.status.clone());
    ...
    for RouterDataV2<F, PaymentFlowData, PaymentsAuthorizeData<T>, PaymentsResponseData>
{
    ...
    fn try_from(
        ...
        match item.response {
            ...
            BraintreePaymentsResponse::AchBankDebitResponse(ach_response) => {
                let transaction_data = &ach_response.charge_us_bank_account.transaction;
                let status = enums::AttemptStatus::from(transaction_data.status.clone());
