...
#[derive(Debug, Clone)]
pub struct Itaubank<T: PaymentMethodDataTypes> {
    payment_method_type: std::marker::PhantomData<T>,
}

impl<T: PaymentMethodDataTypes> Itaubank<T> {
...
impl<T: PaymentMethodDataTypes + Debug + Sync + Send + 'static + Serialize>
    connector_types::RepeatPaymentV2 for Itaubank<T>
{
}
    ...
    > for Itaubank<T>
{
    ...
    fn get_url(
        ...
        let iupipes_env_dns = "";
        Ok(format!(
            "{{https://boleto.${iupipes_env_dns}/boleto/v1}}/payments"
        ))
    }
    ...
    fn get_request_body(
        ...
        let request = itaubank::ItaubankPaymentsRequest::try_from(req)?;
    ...
    > {
        let response: itaubank::ItaubankPaymentsResponse = res
            .response
            .parse_struct("ItaubankPaymentsResponse")
    ...
    for Itaubank<T>
{
    ...
    fn base_url<'a>(&self, _connectors: &'a Connectors) -> &'a str {
        "https://boleto.${iupipes_env_dns}/boleto/v1"
    }
