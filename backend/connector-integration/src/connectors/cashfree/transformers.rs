...
impl UpiFlowType {
    pub fn from_payment_method_data(payment_method_data: &PaymentMethodData) -> Result<Self, ConnectorError> {
    ...
    for CashfreeOrderCreateRequest
{
    ...
    fn try_from(
        ...
        let billing = item.resource_common_data.address.get_payment_method_billing()
        ...
        let customer_details = CashfreeCustomerDetails {
            customer_id: item.resource_common_data.customer_id
                .as_ref()
                .map(|id| id.get_string_repr().to_string())
                .unwrap_or_else(|| "guest".to_string()),
            customer_email: billing.email.as_ref().map(|e| e.peek().to_string()),
            customer_phone: billing.phone.as_ref()
    ...
    for CashfreePaymentRequest
{
    ...
    fn try_from(
        ...
        let payment_session_id = item.resource_common_data.reference_id
            .clone()
            .ok_or(ConnectorError::MissingRequiredField {
...
impl TryFrom<ResponseRouterData<CashfreePaymentResponse, RouterDataV2<Authorize, PaymentFlowData, PaymentsAuthorizeData, PaymentsResponseData>>>
    for RouterDataV2<Authorize, PaymentFlowData, PaymentsAuthorizeData, PaymentsResponseData>
{
    type Error = error_stack::Report<ConnectorError>;
    
    fn try_from(
        item: ResponseRouterData<CashfreePaymentResponse, RouterDataV2<Authorize, PaymentFlowData, PaymentsAuthorizeData, PaymentsResponseData>>,
    ) -> Result<Self, Self::Error> {
        ...
        Ok(Self {
            response: Ok(PaymentsResponseData::TransactionResponse {
                ...
                network_txn_id: None,
                connector_response_reference_id: response.cf_payment_id
                    .map(|id| id.to_string()),
