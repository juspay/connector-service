...
use crate::{connectors::stripe::StripeRouterData, types::ResponseRouterData};

use domain_types::{
    connector_flow::{Authorize, Capture, RepeatPayment, SetupMandate, SubmitEvidence, Void},
    connector_types::{
        DisputeFlowData, DisputeResponseData, MandateReference, MandateReferenceId,
    ...
    payment_method_data::{self, BankRedirectData, PayLaterData},
    payment_method_data::{Card, GooglePayWalletData, PaymentMethodData, WalletData},
    router_data::{self, ConnectorAuthType, PaymentMethodToken},
...
fn create_stripe_payment_method(
    ...
    _is_customer_initiated_mandate_payment: Option<bool>,
    ...
    > for PaymentIntentRequest
{
    ...
    fn try_from(
        ...
        let (
            ...
            let (payment_method_data, payment_method_type, billing_address) =
                create_stripe_payment_method(
                    ...
                    billing_address.ok_or_else(|| ConnectorError::MissingRequiredField {
        ...
        let meta_data = get_transaction_metadata(
        ...
        let charges = None; //Todo split payments not supported yet
        ...
        Ok(Self {
            ...
            currency: item.router_data.request.currency.to_string(), //we need to copy the value and not transfer ownership
            statement_descriptor_suffix: item
                .router_data
                .request
                .statement_descriptor_suffix
                .clone(),
            statement_descriptor: item.router_data.request.statement_descriptor.clone(),
            meta_data,
            return_url: item
                .router_data
                .request
            ...
            capture_method: StripeCaptureMethod::from(item.router_data.request.capture_method),
            payment_data,
            ...
            setup_future_usage: match (
                item.router_data.request.setup_future_usage,
                item.router_data.request.customer_acceptance.as_ref(),
            ) {
                (Some(usage), Some(_)) => Some(usage),
...
fn get_payment_method_type_for_saved_payment_method_payment<F, T>(
    item: &StripeRouterData<RouterDataV2<F, PaymentFlowData, T, PaymentsResponseData>>,
) -> Result<Option<StripePaymentMethodType>, error_stack::Report<ConnectorError>> {
    if item.router_data.resource_common_data.payment_method == common_enums::PaymentMethod::Card {
...
impl<F> TryFrom<StripeRouterData<RouterDataV2<F, RefundFlowData, RefundsData, RefundsResponseData>>>
    for RefundRequest
{
    type Error = error_stack::Report<ConnectorError>;
    fn try_from(
        item: StripeRouterData<RouterDataV2<F, RefundFlowData, RefundsData, RefundsResponseData>>,
    ) -> Result<Self, Self::Error> {
        let payment_intent = item.router_data.request.connector_transaction_id.clone();
        ...
        Ok(Self {
            ...
            meta_data: StripeMetadata {
                order_id: Some(item.router_data.request.refund_id.clone()),
    ...
    > for Evidence
{
    ...
    fn try_from(
        ...
        let submit_evidence_request_data = item.router_data.request.clone();
        Ok(Self {
...
fn get_stripe_payments_response_data(
    ...
    http_code: u16,
    response_id: String,
) -> Box<Result<PaymentsResponseData, router_data::ErrorResponse>> {
    ...
    Box::new(Err(router_data::ErrorResponse {
        code,
...
#[serde(untagged)]
pub enum StripePSyncResponse {
    Payment(PaymentIntentSyncResponse),
    Mandate(SetupIntentResponse),
}
...
impl<F> TryFrom<ResponseRouterData<StripePSyncResponse, Self>>
    for RouterDataV2<F, PaymentFlowData, PaymentsSyncData, PaymentsResponseData>
{
    type Error = error_stack::Report<ConnectorError>;
    fn try_from(value: ResponseRouterData<StripePSyncResponse, Self>) -> Result<Self, Self::Error> {
        match value.response {
            StripePSyncResponse::Mandate(response) => {
            ...
            StripePSyncResponse::Payment(response) => {
                let response_router_data = ResponseRouterData {
                    response,
                    router_data: value.router_data,
                    http_code: value.http_code,
    ...
    > for PaymentIntentRequest
{
    ...
    fn try_from(
        ...
        let mandate_metadata = match &item.router_data.request.mandate_reference {
            MandateReferenceId::ConnectorMandateId(mandate_data) => {
                Some(mandate_data.get_mandate_metadata())
            }
            _ => None,
        ...
        let setup_mandate_details = None;
