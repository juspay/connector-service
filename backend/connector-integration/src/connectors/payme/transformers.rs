...
impl TryFrom<SaleStatus> for RefundStatus {
    ...
    fn try_from(sale_status: SaleStatus) -> Result<Self, Self::Error> {
        match sale_status {
            SaleStatus::Completed | SaleStatus::Refunded | SaleStatus::PartialRefund => Ok(Self::Success),
...
impl<T: PaymentMethodDataTypes + std::fmt::Debug + Sync + Send + 'static + Serialize>
    TryFrom<
        PaymeRouterData<RouterDataV2<Authorize, PaymentFlowData, PaymentsAuthorizeData<T>, PaymentsResponseData>, T>,
    > for PaymePaymentRequest<T>
{
    type Error = error_stack::Report<errors::ConnectorError>;
    ...
    for RouterDataV2<Authorize, PaymentFlowData, PaymentsAuthorizeData<T>, PaymentsResponseData>
{
    ...
    fn try_from(item: ResponseRouterData<PaymePaymentResponse, Self>) -> Result<Self, Self::Error> {
        ...
        if response.status_code != 0 && (response.payme_status != "success" || response.status_error_code.is_some()) {
...
impl TryFrom<&RouterDataV2<PSync, PaymentFlowData, PaymentsSyncData, PaymentsResponseData>> for PaymeSyncRequest {
    type Error = error_stack::Report<errors::ConnectorError>;
...
// Implementation for the macro-generated PaymeRouterData wrapper type
impl<T: PaymentMethodDataTypes + std::fmt::Debug + Sync + Send + 'static + Serialize>
    TryFrom<PaymeRouterData<RouterDataV2<PSync, PaymentFlowData, PaymentsSyncData, PaymentsResponseData>, T>>
    for PaymeSyncRequest
{
    type Error = error_stack::Report<errors::ConnectorError>;

    fn try_from(
        item: PaymeRouterData<RouterDataV2<PSync, PaymentFlowData, PaymentsSyncData, PaymentsResponseData>, T>,
    ) -> Result<Self, Self::Error> {
...
// Implementation for the macro-generated PaymeRouterData wrapper type
impl<T: PaymentMethodDataTypes + std::fmt::Debug + Sync + Send + 'static + Serialize>
    TryFrom<PaymeRouterData<RouterDataV2<Capture, PaymentFlowData, PaymentsCaptureData, PaymentsResponseData>, T>>
    for PaymeCaptureRequest
{
    type Error = error_stack::Report<errors::ConnectorError>;

    fn try_from(
        item: PaymeRouterData<RouterDataV2<Capture, PaymentFlowData, PaymentsCaptureData, PaymentsResponseData>, T>,
    ) -> Result<Self, Self::Error> {
        (&item.router_data).try_into()
    }
}
...
impl TryFrom<&RouterDataV2<Refund, RefundFlowData, RefundsData, RefundsResponseData>> for PaymeRefundRequest {
    type Error = error_stack::Report<errors::ConnectorError>;
...
// Implementation for the macro-generated PaymeRouterData wrapper type
impl<T: PaymentMethodDataTypes + std::fmt::Debug + Sync + Send + 'static + Serialize>
    TryFrom<PaymeRouterData<RouterDataV2<Refund, RefundFlowData, RefundsData, RefundsResponseData>, T>>
    for PaymeRefundRequest
{
    type Error = error_stack::Report<errors::ConnectorError>;

    fn try_from(
        item: PaymeRouterData<RouterDataV2<Refund, RefundFlowData, RefundsData, RefundsResponseData>, T>,
    ) -> Result<Self, Self::Error> {
        (&item.router_data).try_into()
    }
}
...
impl TryFrom<&RouterDataV2<RSync, RefundFlowData, RefundSyncData, RefundsResponseData>> for PaymeRSyncRequest {
    type Error = error_stack::Report<errors::ConnectorError>;
...
// Implementation for the macro-generated PaymeRouterData wrapper type
impl<T: PaymentMethodDataTypes + std::fmt::Debug + Sync + Send + 'static + Serialize>
    TryFrom<PaymeRouterData<RouterDataV2<RSync, RefundFlowData, RefundSyncData, RefundsResponseData>, T>>
    for PaymeRSyncRequest
{
    type Error = error_stack::Report<errors::ConnectorError>;

    fn try_from(
        item: PaymeRouterData<RouterDataV2<RSync, RefundFlowData, RefundSyncData, RefundsResponseData>, T>,
    ) -> Result<Self, Self::Error> {
        (&item.router_data).try_into()
    }
}
...
impl TryFrom<&RouterDataV2<Void, PaymentFlowData, PaymentVoidData, PaymentsResponseData>> for PaymeVoidRequest {
    type Error = error_stack::Report<errors::ConnectorError>;
...
// Implementation for the macro-generated PaymeRouterData wrapper type
impl<T: PaymentMethodDataTypes + std::fmt::Debug + Sync + Send + 'static + Serialize>
    TryFrom<PaymeRouterData<RouterDataV2<Void, PaymentFlowData, PaymentVoidData, PaymentsResponseData>, T>>
    for PaymeVoidRequest
{
    type Error = error_stack::Report<errors::ConnectorError>;

    fn try_from(
        item: PaymeRouterData<RouterDataV2<Void, PaymentFlowData, PaymentVoidData, PaymentsResponseData>, T>,
    ) -> Result<Self, Self::Error> {
        (&item.router_data).try_into()
    }
}
    ...
    for PaymeGenerateSaleRequest
{
    ...
    fn try_from(
        ...
        Ok(Self {
            ...
            transaction_id: item.resource_common_data.connector_request_reference_id.clone(),
    ...
    for RouterDataV2<CreateOrder, PaymentFlowData, PaymentCreateOrderData, PaymentCreateOrderResponse>
{
    ...
    fn try_from(item: ResponseRouterData<PaymeGenerateSaleResponse, Self>) -> Result<Self, Self::Error> {
        let response = item.response;

        // Check for error responses
        if response.status_code != 0 {
            let error_message = format!("Order creation failed with status code: {}", response.status_code);
