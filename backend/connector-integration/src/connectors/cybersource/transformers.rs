...
fn get_card_number_string<T: PaymentMethodDataTypes + 'static>(
    ...
    {
        let default_card_number = unsafe {
            std::mem::transmute::<
                &RawCardNumber<T>,
                &RawCardNumber<payment_method_data::DefaultPCIHolder>,
            >(card_number)
        };
        default_card_number.peek().to_string()
    } else {
        let vault_card_number = unsafe {
            std::mem::transmute::<
                &RawCardNumber<T>,
                &RawCardNumber<payment_method_data::VaultTokenHolder>,
            >(card_number)
        };
        vault_card_number.peek().to_string()
    }
}
                ...
                .and_then(|mandate_id| mandate_id.mandate_reference_id)
            {
                Some(MandateReferenceId::ConnectorMandateId(_)) => {
                    let original_amount = item
                        ...
                        .as_ref()
                        .and_then(|metadata| metadata.as_object())
                        .and_then(|obj| obj.get("original_payment_authorized_amount"))
                    ...
                    let original_currency = item
                        ...
                        .as_ref()
                        .and_then(|metadata| metadata.as_object())
                        .and_then(|obj| obj.get("original_payment_authorized_currency"))
                        .and_then(|val| val.as_str())
                ...
                Some(MandateReferenceId::NetworkMandateId(network_transaction_id)) => {
                        ...
                        .as_deref()
                    {
                        ...
                        Some("004") => {
                            let original_amount = item.router_data.request.amount;
                    ...
                    let original_amount = Some(original_amount);
                    let original_currency = Some(original_currency);
                    let original_authorized_amount = match original_amount.zip(original_currency) {
                ...
                Some(MandateReferenceId::NetworkTokenWithNTI(mandate_data)) => {
                    let (original_amount, original_currency) = match network
                        .clone()
                        .map(|network| network.to_lowercase())
                        .as_deref()
    ...
    )> for ProcessingInformation
{
    ...
    fn try_from(
        ...
        let (action_list, action_token_types, authorization_options) =
            if item.router_data.request.setup_future_usage == Some(common_enums::FutureUsage::OffSession)
    ...
    )> for CybersourcePaymentsRequest<T>
{
    ...
    fn try_from(
        ...
        let raw_card_number: RawCardNumber<T> = unsafe {
            std::mem::transmute_copy(&RawCardNumber::<payment_method_data::DefaultPCIHolder>(
                ccard.card_number,
            ))
        };
...
impl< T: PaymentMethodDataTypes
            ...
            + 'static
            + Serialize,>
 TryFrom<&CybersourceRouterData<&PaymentsPreProcessingRouterData>>
    for CybersourcePreProcessingRequest<T>
{
    type Error = error_stack::Report<errors::ConnectorError>;
    fn try_from(
        ...
        let payment_information = match payment_method_data {
            PaymentMethodData::Card(ccard) => {
                ...
                {
                    Some(card_network) => Some(card_network.to_string()),
                    None => domain_types::utils::get_card_issuer(&ccard.card_number.peek()).ok().map(String::from),
                };
        ...
        Ok(Self {
            response: match item.response.error_information {
                ...
                None => Ok(PaymentsResponseData::IncrementalAuthorizationResponse {
                    status: item.router_data.status.into(),
                    error_code: None,
                    error_message: None,
                    connector_authorization_id: None,
                }),
    ...
    for CybersourcePayoutFulfillRequest
{
    ...
    fn try_from(
        ...
        match payout_type {
            ...
            common_enums::PayoutType::Bank | common_enums::PayoutType::Wallet => {
    ...
    fn try_from(item: PayoutMethodData) -> Result<Self, Self::Error> {
        match item {
            PayoutMethodData::Card(card_details) => {
                let card_issuer = domain_types::utils::get_card_issuer(&card_details.card_number.peek()).ok();
