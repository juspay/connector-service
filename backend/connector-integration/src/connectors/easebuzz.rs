...
use domain_types::{
    connector_flow::{
        Accept, Authenticate, Authorize, Capture, CreateAccessToken, CreateOrder, CreateSessionToken,
        DefendDispute, PaymentMethodToken, PostAuthenticate, PreAuthenticate, PSync, RSync,
        Refund, RepeatPayment, SetupMandate, SubmitEvidence, Void,
    },
    connector_types::{
        AcceptDisputeData, ConnectorAccessTokenData, ConnectorCustomerData, ConnectorWebhookSecrets,
        DisputeDefendData, DisputeFlowData, DisputeResponseData, PaymentCreateOrderData,
        PaymentCreateOrderResponse, PaymentFlowData, PaymentMethodTokenizationData,
        PaymentMethodTokenResponse, PaymentVoidData, PaymentsAuthorizeData, PaymentsAuthenticateData,
...
use transformers::{
    ...
    EaseBuzzPaymentsSyncResponse, EaseBuzzRefundSyncRequest, EaseBuzzRefundSyncResponse,
    EaseBuzzVoidRequest, EaseBuzzVoidResponse, EaseBuzzCaptureRequest, EaseBuzzCaptureResponse,
    EaseBuzzRefundRequest, EaseBuzzRefundResponse, EaseBuzzCreateOrderRequest,
    EaseBuzzCreateOrderResponse, EaseBuzzSessionTokenRequest, EaseBuzzSessionTokenResponse,
    EaseBuzzSetupMandateRequest, EaseBuzzSetupMandateResponse, EaseBuzzRepeatPaymentRequest,
    EaseBuzzRepeatPaymentResponse, EaseBuzzAcceptDisputeRequest, EaseBuzzAcceptDisputeResponse,
    EaseBuzzDefendDisputeRequest, EaseBuzzDefendDisputeResponse, EaseBuzzSubmitEvidenceRequest,
    ...
    > connector_types::IncomingWebhook for EaseBuzz<T>
{
    ...
    fn process_payment_webhook(
        ...
        error_stack::Report<domain_types::errors::ConnectorError>,
    > {
        // TODO: Implement webhook processing based on EaseBuzz requirements
...
macros::macro_connector_implementation!(
    ...
    other_functions: {
        fn get_headers(
            ...
            let mut auth_header = get_auth_header_easebuzz(&auth_type)?;
...
macros::macro_connector_implementation!(
    ...
    other_functions: {
        ...
        fn get_url(
            ...
            let is_test = req.resource_common_data.test_mode.unwrap_or(false);
            let endpoint = if is_test {
                "https://testpay.easebuzz.in/transaction/v1/sync"
            } else {
                "https://pay.easebuzz.in/transaction/v1/sync"
            };
            Ok(endpoint.to_string())
        }
    }
...
macros::macro_connector_implementation!(
    ...
    other_functions: {
        ...
        fn get_url(
            ...
            let is_test = req.resource_common_data.test_mode.unwrap_or(false);
            let endpoint = if is_test {
                "https://testpay.easebuzz.in/transaction/v1/refund/sync"
            } else {
                "https://pay.easebuzz.in/transaction/v1/refund/sync"
            };
            Ok(endpoint.to_string())
        }
    }
...
fn get_auth_header_easebuzz(
    ...
    Ok(vec![(headers::AUTHORIZATION.to_string(), format!("{}:{}", api_key.peek(), salt.peek()).into_masked())])
}