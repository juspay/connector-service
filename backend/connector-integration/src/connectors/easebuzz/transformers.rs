...
impl TryFrom<&ConnectorAuthType> for EaseBuzzAuth {
    ...
    fn try_from(auth_type: &ConnectorAuthType) -> Result<Self, Self::Error> {
        match auth_type {
            ConnectorAuthType::SignatureKey { api_key, key_secret } => Ok(Self {
                key: api_key.clone(),
                salt: key_secret.clone(),
                merchant_id: None,
            }),
            ConnectorAuthType::MultiAccountingKey { api_key, key_secret, .. } => Ok(Self {
                key: api_key.clone(),
                salt: key_secret.clone(),
                merchant_id: None,
            }),
...
impl<F, T: PaymentMethodDataTypes + std::fmt::Debug + std::marker::Sync + std::marker::Send + 'static + Serialize>
    TryFrom<ResponseRouterData<EaseBuzzPaymentsResponseEnum, Self>>
    for RouterDataV2<F, PaymentFlowData, PaymentsAuthorizeData<T>, PaymentsResponseData>
{
    type Error = error_stack::Report<ConnectorError>;

    fn try_from(
        item: ResponseRouterData<EaseBuzzPaymentsResponseEnum, Self>,
    ) -> Result<Self, Self::Error> {
