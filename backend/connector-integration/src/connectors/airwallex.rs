...
use serde::Serialize;
use transformers::{
    self as airwallex, AirwallexAccessTokenRequest, AirwallexAccessTokenResponse, AirwallexCaptureRequest,
    AirwallexCaptureResponse, AirwallexIntentRequest, AirwallexIntentResponse, AirwallexPaymentRequest,
    AirwallexPaymentsResponse, AirwallexRefundRequest, AirwallexRefundResponse, AirwallexRefundSyncResponse,
...
impl<T: PaymentMethodDataTypes + Debug + Sync + Send + 'static + Serialize> connector_types::RefundV2 for Airwallex<T> {}
...
impl<T: PaymentMethodDataTypes + Debug + Sync + Send + 'static + Serialize> SourceVerification for Airwallex<T> {}

impl<T: PaymentMethodDataTypes + Debug + Sync + Send + 'static + Serialize> BodyDecoding for Airwallex<T> {}
...
// Dispute Accept
impl<T: PaymentMethodDataTypes + Debug + Sync + Send + 'static + Serialize>
    ConnectorIntegrationV2<Accept, DisputeFlowData, AcceptDisputeData, DisputeResponseData> for Airwallex<T>
{
}

// Dispute Defend
impl<T: PaymentMethodDataTypes + Debug + Sync + Send + 'static + Serialize>
    ConnectorIntegrationV2<DefendDispute, DisputeFlowData, DisputeDefendData, DisputeResponseData> for Airwallex<T>
{
}

// Submit Evidence
impl<T: PaymentMethodDataTypes + Debug + Sync + Send + 'static + Serialize>
    ConnectorIntegrationV2<SubmitEvidence, DisputeFlowData, SubmitEvidenceData, DisputeResponseData> for Airwallex<T>
{
}
...
impl<T: PaymentMethodDataTypes + Debug + Sync + Send + 'static + Serialize> ConnectorCommon for Airwallex<T> {
    ...
    fn get_auth_header(
        ...
        let auth = airwallex::AirwallexAuthType::try_from(auth_type)
            .change_context(errors::ConnectorError::FailedToObtainAuthType)?;
        Ok(vec![
            ("x-api-key".to_string(), auth.api_key.expose().to_string().into()),
            ("x-client-id".to_string(), auth.client_id.expose().to_string().into()),
        ])
    }
