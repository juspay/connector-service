...
// Trait implementations with generic type parameters - all required traits for ConnectorServiceTrait
impl<
...
// Add required ConnectorIntegrationV2 implementations - stubs for all required flows
impl<
    T: PaymentMethodDataTypes
        + std::fmt::Debug
        + std::marker::Sync
...
// Add SourceVerification implementations for all flows - stub implementations
impl<F, FCD, Req, Res, T> SourceVerification<F, FCD, Req, Res> for Billdesk<T>
where
    T: PaymentMethodDataTypes
        + std::fmt::Debug
        + std::marker::Sync
        ...
        + Serialize,
{
    ...
    fn get_signature(
        &self,
        _payload: &[u8],
        _router_data: &RouterDataV2<F, FCD, Req, Res>,
        _secrets: &[u8],
    ) -> CustomResult<Vec<u8>, errors::ConnectorError> {
        Ok(Vec::new())
    }
