...
use domain_types::{
    connector_flow::{
        Accept, Authenticate, Authorize, Capture, CreateAccessToken, CreateConnectorCustomer, CreateOrder, CreateSessionToken, DefendDispute, PSync, PaymentMethodToken, PostAuthenticate, PreAuthenticate, RSync,
    ...
    connector_types::{
        ...
        PaymentVoidData, PaymentsAuthenticateData, PaymentsAuthorizeData, PaymentsCaptureData, PaymentsPostAuthenticateData,
...
> ConnectorCommon for Billdesk<T>
{
    ...
    fn get_auth_header(
        ...
        // Billdesk uses custom checksum authentication in get_headers
        Ok(vec![])
    }
...
fn generate_billdesk_checksum<T: PaymentMethodDataTypes + Debug + Sync + Send + 'static + Serialize>(
    req: &RouterDataV2<Authorize, PaymentFlowData, PaymentsAuthorizeData<T>, PaymentsResponseData>,
    auth_type: &transformers::BilldeskAuth,
) -> CustomResult<String, errors::ConnectorError> {
    // TODO: Implement Billdesk checksum generation logic
    // This should create a checksum based on the request data and the checksum key
    ...
    // Simple hash for now - replace with actual Billdesk checksum algorithm
