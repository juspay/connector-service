...
macros::macro_connector_implementation!(
    ...
    other_functions: {
        fn get_headers(
            ...
            let auth_type = billdesk::BilldeskAuth::try_from(&req.connector_auth_type)?;

            let auth_header = get_billdesk_auth_header(&auth_type)?;
...
fn get_billdesk_auth_header(
    auth_type: &billdesk::BilldeskAuth,
) -> CustomResult<Maskable<String>, errors::ConnectorError> {
    let merchant_id = auth_type.merchant_id.as_ref()
        .ok_or(errors::ConnectorError::FailedToObtainAuthType)?;
    let checksum_key = auth_type.checksum_key.as_ref()
        .ok_or(errors::ConnectorError::FailedToObtainAuthType)?;

    // Billdesk uses custom authentication format
