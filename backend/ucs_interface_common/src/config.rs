...
) -> CustomResult<Arc<configs::Config>, ConnectorRequestError> {
            ...
            let mut override_patch: ConfigPatch = serde_json::from_str(config_override.trim())
                .map_err(|e| {
                    Report::new(ConnectorRequestError::config_error(
                        ...
                        format!("Cannot convert override config to JSON: {e}"),
                    ))
                })?;
                    ...
                    .and_then(|value| value.as_ref())
                {
                    ...
                    let cert = if cert_trimmed.is_empty() {
                        Err(Report::new(ConnectorRequestError::config_error(
                            "INVALID_MITM_CA_CERT_BASE64",
                            "proxy.mitm_ca_cert must be base64-encoded",
                        )))
                    } else {
                        ...
                        let decoded = general_purpose::STANDARD
                            ...
                            .map_err(|e| {
                                Report::new(ConnectorRequestError::config_error(
                                    "INVALID_MITM_CA_CERT_BASE64",
                                    format!("Invalid base64 for proxy.mitm_ca_cert: {e}"),
                                ))
                            })?;

                        String::from_utf8(decoded).map_err(|e| {
                            Report::new(ConnectorRequestError::config_error(
                                "INVALID_MITM_CA_CERT_UTF8",
                                format!("Decoded proxy.mitm_ca_cert is not valid UTF-8: {e}"),
                            ))
                        })
                    }?;
...
) -> CustomResult<Connectors, ConnectorRequestError> {
