...
const TEST_DATATRANS_API_KEY_ENV: &str = "TEST_DATATRANS_API_KEY";
...
// Helper function to add datatrans metadata headers to a request
fn add_datatrans_metadata<T>(request: &mut Request<T>) {
    // Get API credentials from environment variables
    let api_key = env::var(TEST_DATATRANS_API_KEY_ENV)
        .unwrap_or_else(|_| panic!("Environment variable TEST_DATATRANS_API_KEY_ENV must be set"));
    let key1 = env::var(TEST_DATATRANS_KEY1_ENV)
        .unwrap_or_else(|_| panic!("Environment variable TEST_DATATRANS_KEY1_ENV must be set"));
...
fn create_payment_authorize_request(
    ...
    request_ref_id.id_type = Some(IdType::Id(format!("datatrans_test_{}", get_timestamp())));
...
fn create_payment_sync_request(transaction_id: &str) -> PaymentServiceGetRequest {
    PaymentServiceGetRequest {
        ...
        request_ref_id: Some(Identifier {
            id_type: Some(IdType::Id(format!("datatrans_sync_{}", get_timestamp()))),
        }),
        access_token: None,
        capture_method: None,
...
async fn test_payment_authorization_manual_capture() {
    grpc_test!(client, PaymentServiceClient<Channel>, {
        ...
        add_datatrans_metadata(&mut auth_grpc_request);
...
async fn test_payment_sync() {
    grpc_test!(client, PaymentServiceClient<Channel>, {
        ...
        add_datatrans_metadata(&mut sync_grpc_request);
...
async fn test_payment_capture() {
    grpc_test!(client, PaymentServiceClient<Channel>, {
        ...
        add_datatrans_metadata(&mut capture_grpc_request);
...
async fn test_refund() {
    grpc_test!(client, PaymentServiceClient<Channel>, {
        ...
        add_datatrans_metadata(&mut refund_grpc_request);
...
async fn test_refund_sync() {
    grpc_test!(client, PaymentServiceClient<Channel>, {
        grpc_test!(refund_client, RefundServiceClient<Channel>, {
            ...
            add_datatrans_metadata(&mut refund_sync_grpc_request);
...
async fn test_payment_void() {
    grpc_test!(client, PaymentServiceClient<Channel>, {
        ...
        add_datatrans_metadata(&mut void_grpc_request);
    ...
    grpc_test!(client, PaymentServiceClient<Channel>, {
        ...
        if let Some(mandate_ref) = &response.mandate_reference {
            ...
            if let Some(mandate_id) = &mandate_ref.mandate_id {
                assert!(
                    !mandate_id.is_empty(),
                    "Mandate ID should not be empty"
                );
            }
        }
