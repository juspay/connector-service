...
use hyperswitch_common_utils::access_token;
use std::env;
...
const CONNECTOR_NAME: &str = "paypal";

// Environment variable names for API credentials (can be set or overridden with provided values)
const PAYPAL_ACCESS_TOKEN_ENV: &str = "TEST_PAYPAL_ACCESS_TOKEN";

// Test card data
const TEST_AMOUNT: i64 = 10000;
const TEST_CARD_NUMBER: &str = "4000000000001091"; // Valid test card for Paypal
...
fn add_paypal_metadata<T>(request: &mut Request<T>) {
    ...
    request.metadata_mut().append(
        "x-auth",
        "temporary-auth".parse().expect("Failed to parse x-auth"),
    );
}
...
fn create_payment_authorize_request(capture_method: CaptureMethod) -> PaymentsAuthorizeRequest {
    let access_token =
        env::var(PAYPAL_ACCESS_TOKEN_ENV).expect("TEST_PAYPAL_ACCESS_TOKEN environment variable is required");
    ...
    PaymentsAuthorizeRequest {
        ...
        connector_request_reference_id: format!("paypal_test_{}", get_timestamp()),
...
fn create_payment_sync_request(transaction_id: &str) -> PaymentsSyncRequest {
    PaymentsSyncRequest {
        ...
        connector_request_reference_id: Some(format!("paypal_sync_{}", get_timestamp())),
    }
}
...
async fn test_payment_authorization_auto_capture() {
    grpc_test!(client, PaymentServiceClient<Channel>, {
        // Add delay of 5 seconds to ensure connector_request_reference_id is different
        ...
        add_paypal_metadata(&mut grpc_request);
...
async fn test_payment_authorization_manual_capture() {
    grpc_test!(client, PaymentServiceClient<Channel>, {
        ...
        add_paypal_metadata(&mut auth_grpc_request);
