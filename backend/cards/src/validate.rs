...
impl CardNumber {
    ...
    pub fn is_cobadged_card(&self) -> Result<bool, error_stack::Report<ValidationError>> {
        ...
        static CARD_NETWORK_REGEX: LazyLock<HashMap<&str, Result<Regex, regex::Error>>> = LazyLock::new(|| {
            ...
            map.insert(
                "Discover",
                Regex::new(r"^(6011|64[4-9]|65|622126|622[1-9][0-9][0-9]|6229[0-1][0-9]|622925)"),
            );
            map.insert(
        "Maestro",
...
pub fn validate_card_number_chars(number: &str) -> Result<Vec<u8>, CardNumberValidationErr> {
    let data = number
        .chars()
        .try_fold(Vec::with_capacity(MAX_CARD_NUMBER_LENGTH), |mut data, character| {
            data.push(
                #[allow(clippy::expect_used)]
                character
                    .to_digit(10)
                    .ok_or(CardNumberValidationErr("invalid character found in card number"))?
                    .try_into()
...
pub fn luhn(number: &[u8]) -> bool {
    number
        ...
        .enumerate()
        .map(|(idx, element)| {
            ((*element * 2) / 10 + (*element * 2) % 10) * ((idx as u8) % 2) + (*element) * (((idx + 1) as u8) % 2)
        })
        .sum::<u8>()
        % 10
        == 0
...
mod tests {
    ...
    fn valid_card_number() {
        ...
        assert_eq!(CardNumber::from_str(s).unwrap(), CardNumber(StrongSecret::from_str(s).unwrap()));
    }
    ...
    fn card_number_no_whitespace() {
        ...
        assert_eq!(CardNumber::from_str(s).unwrap().to_string(), "371449*********");
    }

    #[test]
    fn test_valid_card_number_masking() {
        let secret: Secret<String, CardNumberStrategy> = Secret::new("1234567890987654".to_string());
