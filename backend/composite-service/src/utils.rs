...
use grpc_api_types::payments::{
    AccessToken, CustomerServiceCreateResponse, MerchantAuthenticationServiceCreateAccessTokenResponse,
};
...
pub fn connector_from_composite_authorize_metadata(
    ...
    metadata
        ...
        .ok_or_else(|| Box::new(tonic::Status::invalid_argument("missing x-connector metadata")))
        .and_then(|connector| {
            connector
                .to_str()
                .map_err(|_| Box::new(tonic::Status::invalid_argument("invalid x-connector metadata value")))
        })
        .and_then(|connector_from_metadata| {
            ConnectorEnum::from_str(connector_from_metadata)
                .map_err(|err| Box::new(tonic::Status::invalid_argument(format!("Connector not supported: {err}"))))
        })
}
...
pub fn grpc_connector_from_connector_enum(connector: &ConnectorEnum) -> i32 {
    ...
    let grpc_connector = grpc_api_types::payments::Connector::from_str_name(grpc_connector_name.as_str())
...
pub fn get_access_token(
    ...
    access_token_from_request.or_else(|| access_token_from_create_access_token_response(access_token_response))
}
