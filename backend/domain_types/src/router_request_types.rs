...
impl TryFrom<payments::AuthenticationData> for AuthenticationData {
    type Error = error_stack::Report<errors::ConnectorRequestError>;
    fn try_from(value: payments::AuthenticationData) -> Result<Self, Self::Error> {
        ...
        let message_version = message_version
            .map(|message_version| {
                SemanticVersion::from_str(&message_version).change_context(
                    errors::ConnectorRequestError::config_error(
                        "INVALID_SEMANTIC_VERSION_DATA",
        ...
        let trans_status = trans_status
            .map(|trans_status| {
                payments::TransactionStatus::try_from(trans_status).change_context(
                    errors::ConnectorRequestError::config_error(
                        "INVALID_TRANSACTION_STATUS",
                        "Invalid transaction status format. Expected one of the valid 3DS transaction status values",
                    ),
                )
            })
            .transpose()?
...
impl TryFrom<payments::CartesBancairesParams> for CartesBancairesParams {
    ...
    fn try_from(value: payments::CartesBancairesParams) -> Result<Self, Self::Error> {
        let cavv_algorithm = payments::CavvAlgorithm::try_from(value.cavv_algorithm)
            ...
            .ok_or_else(|| {
                error_stack::report!(errors::ConnectorRequestError::config_error(
                    "INVALID_CAVV_ALGORITHM",
