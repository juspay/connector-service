...
/// Convert Spanish autonomous community/province names to their official codes
pub fn convert_spain_state_to_code(
    state: &str,
) -> error_stack::Result<String, errors::ConnectorError> {
    // If already 3 characters or less, assume it's already a code
    if state.len() <= 3 {
        return Ok(state.to_uppercase());
    }

    // Convert full region names to codes (case-insensitive, normalized without accents/spaces)
    let normalized_state = state.to_lowercase().replace(&[' ', '-', '.'][..], "");
