...
pub fn ffi_headers_to_masked_metadata(headers: &HashMap<String, String>) -> Result<MaskedMetadata, FfiError> {
    ucs_interface_common::headers::headers_to_masked_metadata(headers, HeaderMaskingConfig::default()).map_err(|e| {
        match e {
...
pub fn load_config(embedded_config: &str) -> Result<Arc<Config>, FfiError> {
    toml::from_str(embedded_config)
        .map(Arc::new)
        .map_err(|e| FfiError::ConfigError { message: e.to_string() })
}
