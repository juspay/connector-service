# Connector `stripe` / Suite `incremental_authorization` / Scenario `incremental_auth_multiple_increase`
...
```text
assertion failed for field 'status': expected one of ["SUCCESS", "AUTHORIZED", "FAILED"], got "AUTHORIZATION_FAILURE"
```
...
grpcurl -plaintext \
  -H "x-merchant-id: test_merchant" \
  -H "x-tenant-id: default" \
  -H "x-request-id: incremental_authorization_incremental_auth_multiple_increase_req" \
  -H "x-connector-request-reference-id: incremental_authorization_incremental_auth_multiple_increase_ref" \
  -H "x-connector-config: ***MASKED***" \
...
Request metadata to send:
x-connector-config: ***MASKED***
x-connector-request-reference-id: incremental_authorization_incremental_auth_multiple_increase_ref
x-merchant-id: test_merchant
x-request-id: incremental_authorization_incremental_auth_multiple_increase_req
x-tenant-id: default

Response headers received:
content-type: application/grpc
date: Tue, 24 Mar 2026 08:50:31 GMT
x-request-id: incremental_authorization_incremental_auth_multiple_increase_req
...
Response contents:
{
  ...
  "responseHeaders": {
    ...
    "content-type": "application/json",
    "date": "Tue, 24 Mar 2026 08:50:31 GMT",
    "idempotency-key": "8989c779-a11f-4d56-909b-fdbef476096f",
    "original-request": "req_wdMI7p0PhjPCfX",
    "request-id": "req_wdMI7p0PhjPCfX",
    "server": "nginx",
    "strict-transport-security": "max-age=63072000; includeSubDomains; preload",
    "stripe-version": "2022-11-15",
    "vary": "Origin",
