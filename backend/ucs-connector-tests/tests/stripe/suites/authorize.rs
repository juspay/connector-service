...
use ucs_connector_tests::harness::{
    assertions, base_requests, context::FlowContext, executor::StripeExecutor,
};

use crate::stripe::suites::generated_input_variants;
...
pub async fn execute(
    ...
    let mut request = base_requests::base_authorize_request_for_connector("stripe", &context.case);
    ...
    let response = executor
        ...
        .await
        .expect("stripe authorize should return a response")
        .into_inner();

    context.capture_from_authorize_response(&response);
    assert_expectation(&response, expectation, "stripe authorize");
}

/// @capability capability_id=STP-CAP-001
...
#[tokio::test]
#[serial]
async fn test_stripe__suite_authorize__no3ds_auto_capture__returns_expected_outcome() {
    let executor = StripeExecutor::new().await;

    for case in generated_input_variants() {
        let mut context = FlowContext::new(case, "stripe_authorize_no3ds_auto_suite");
        execute(
            &executor,
            "stripe_authorize_no3ds_auto_suite",
            &mut context,
...
/// @capability capability_id=STP-CAP-002
/// @capability connector=stripe
/// @capability layer=suite
/// @capability flow=authorize
/// @capability payment_method=card
...
#[tokio::test]
#[serial]
async fn test_stripe__suite_authorize__no3ds_manual_capture__returns_expected_outcome() {
    let executor = StripeExecutor::new().await;

    for case in generated_input_variants() {
        let mut context = FlowContext::new(case, "stripe_authorize_no3ds_manual_suite");
        execute(
            &executor,
            "stripe_authorize_no3ds_manual_suite",
            &mut context,
            AuthorizeScenario::No3dsManualCapture,
        )
        .await;
...
/// @capability capability_id=STP-CAP-003
/// @capability connector=stripe
/// @capability layer=suite
/// @capability flow=authorize
/// @capability payment_method=card
...
/// @capability expected=status=FAILURE_with_decline_error_signal
#[tokio::test]
#[serial]
async fn test_stripe__suite_authorize__no3ds_fail_payment__returns_expected_outcome() {
    let executor = StripeExecutor::new().await;

    for case in generated_input_variants() {
        let mut context = FlowContext::new(case, "stripe_authorize_no3ds_fail_suite");
        execute(
            &executor,
            "stripe_authorize_no3ds_fail_suite",
            &mut context,
