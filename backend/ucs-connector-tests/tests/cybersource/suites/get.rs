...
pub async fn execute(executor: &CybersourceExecutor, flow_name: &str, context: &mut FlowContext) {
    ...
    let transaction_id = context.require_connector_transaction_id("cybersource get");

    for attempt in 0..expectation.max_attempts {
        let mut request = base_requests::get_request_for_connector(
            "cybersource",
            &transaction_id,
        ...
        let response = executor
            ...
            .await
            .expect("cybersource get should return a response")
            .into_inner();
    ...
    panic!("cybersource get did not settle to an expected success status");
}

/// @capability capability_id=CYB-CAP-006
...
#[tokio::test]
#[serial]
async fn test_cybersource__suite_get__after_manual_authorize_and_capture__returns_success_sync_status(
) {
    let executor = CybersourceExecutor::new().await;

    for case in generated_input_variants() {
        let mut context = FlowContext::new(case, "cybersource_get_suite");
        authorize::execute(
            &executor,
            "cybersource_get_suite",
            &mut context,
        ...
        .await;
        execute(&executor, "cybersource_get_suite", &mut context).await;
    }
}
