...
use ucs_connector_tests::harness::{
    assertions, base_requests, context::FlowContext, executor::CybersourceExecutor,
};

use crate::cybersource::suites::generated_input_variants;
...
pub struct AuthorizeOverrides {
    ...
    pub billing_state_override: Option<&'static str>,
}
...
fn scenario_overrides(scenario: AuthorizeScenario) -> AuthorizeOverrides {
    match scenario {
        ...
        AuthorizeScenario::No3dsFailPayment => AuthorizeOverrides {
            ...
            auth_type: AuthenticationType::NoThreeDs,
            enrolled_for_3ds: false,
            billing_state_override: Some("Washingtonr"),
        },
    }
}
...
fn scenario_expectation(scenario: AuthorizeScenario) -> AuthorizeExpectation {
    match scenario {
        ...
        AuthorizeScenario::No3dsFailPayment => AuthorizeExpectation {
            ...
            error_message_contains: Some("invalid"),
        },
    }
}
...
fn apply_overrides(request: &mut PaymentServiceAuthorizeRequest, overrides: AuthorizeOverrides) {
    ...
    if let Some(state) = overrides.billing_state_override {
        if let Some(address) = request
            .address
            .as_mut()
            .and_then(|payment_address| payment_address.billing_address.as_mut())
        {
            address.state = Some(state.to_string().into());
        }
    }
}
...
pub async fn execute(
    ...
    let mut request =
        base_requests::base_authorize_request_for_connector("cybersource", &context.case);
    ...
    let response = executor
        ...
        .await
        .expect("cybersource authorize should return a response")
        .into_inner();

    context.capture_from_authorize_response(&response);
    assert_expectation(&response, expectation, "cybersource authorize");
}

/// @capability capability_id=CYB-CAP-001
...
#[tokio::test]
#[serial]
async fn test_cybersource__suite_authorize__no3ds_auto_capture__returns_expected_outcome() {
    let executor = CybersourceExecutor::new().await;

    for case in generated_input_variants() {
        let mut context = FlowContext::new(case, "cybersource_authorize_no3ds_auto_suite");
        execute(
            &executor,
            "cybersource_authorize_no3ds_auto_suite",
            &mut context,
...
/// @capability capability_id=CYB-CAP-002
/// @capability connector=cybersource
/// @capability layer=suite
/// @capability flow=authorize
/// @capability payment_method=card
...
#[tokio::test]
#[serial]
async fn test_cybersource__suite_authorize__no3ds_manual_capture__returns_expected_outcome() {
    let executor = CybersourceExecutor::new().await;

    for case in generated_input_variants() {
        let mut context = FlowContext::new(case, "cybersource_authorize_no3ds_manual_suite");
        execute(
            &executor,
            "cybersource_authorize_no3ds_manual_suite",
            &mut context,
            AuthorizeScenario::No3dsManualCapture,
        )
        .await;
...
/// @capability capability_id=CYB-CAP-003
/// @capability connector=cybersource
/// @capability layer=suite
/// @capability flow=authorize
/// @capability payment_method=card
...
/// @capability expected=status=FAILURE_with_invalid_data_error_signal
#[tokio::test]
#[serial]
async fn test_cybersource__suite_authorize__no3ds_fail_payment__returns_expected_outcome() {
    let executor = CybersourceExecutor::new().await;

    for case in generated_input_variants() {
        let mut context = FlowContext::new(case, "cybersource_authorize_no3ds_fail_suite");
        execute(
            &executor,
            "cybersource_authorize_no3ds_fail_suite",
            &mut context,
