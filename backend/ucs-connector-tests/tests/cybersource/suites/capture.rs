...
pub async fn execute(
    ...
    let transaction_id = context.require_connector_transaction_id("cybersource capture");
    let mut request = base_requests::capture_request_for_connector(
        "cybersource",
        &transaction_id,
        overrides.amount_to_capture,
    );
    ...
    let response = executor
        ...
        .await
        .expect("cybersource capture should return a response")
        .into_inner();

    assert!(
        response.error.is_none(),
        "cybersource capture should not have errors"
    );
    ...
    assert!(
        is_allowed,
        "cybersource capture returned unexpected status {}",
        response.status
    );
...
/// @capability capability_id=CYB-CAP-004
...
#[tokio::test]
#[serial]
async fn test_cybersource__suite_capture__full_capture_after_manual_authorize__returns_expected_capture_status(
) {
    let executor = CybersourceExecutor::new().await;

    for case in generated_input_variants() {
        let mut context = FlowContext::new(case, "cybersource_capture_full_suite");
        authorize::execute(
            &executor,
            "cybersource_capture_full_suite",
            &mut context,
...
/// @capability capability_id=CYB-CAP-005
/// @capability connector=cybersource
/// @capability layer=suite
/// @capability flow=capture
/// @capability payment_method=card
...
#[tokio::test]
#[serial]
async fn test_cybersource__suite_capture__partial_capture_after_manual_authorize__returns_expected_partial_capture_status(
) {
    let executor = CybersourceExecutor::new().await;

    for case in generated_input_variants() {
        let mut context = FlowContext::new(case, "cybersource_capture_partial_suite");
        authorize::execute(
            &executor,
            "cybersource_capture_partial_suite",
            &mut context,
            authorize::default_manual_scenario(),
        )
        .await;
