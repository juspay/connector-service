...
pub async fn execute(
    ...
    let transaction_id = context.require_connector_transaction_id("adyen capture");
    ...
    let response = executor
        ...
        .await
        .expect("adyen capture should return a response")
        .into_inner();

    assert!(
        response.error.is_none(),
        "adyen capture should not have errors"
    );
    ...
    assert!(
        is_allowed,
        "adyen capture returned unexpected status {}",
        response.status
    );
...
/// @capability capability_id=ADY-CAP-004
...
#[tokio::test]
#[serial]
async fn test_adyen__suite_capture__full_capture_after_manual_authorize__returns_expected_capture_status(
) {
    let executor = AdyenExecutor::new().await;

    for case in generated_input_variants() {
        let mut context = FlowContext::new(case, "adyen_capture_full_suite");
        authorize::execute(
            &executor,
            "adyen_capture_full_suite",
            &mut context,
...
/// @capability capability_id=ADY-CAP-005
/// @capability connector=adyen
/// @capability layer=suite
/// @capability flow=capture
/// @capability payment_method=card
...
#[tokio::test]
#[serial]
async fn test_adyen__suite_capture__partial_capture_after_manual_authorize__returns_expected_partial_capture_status(
) {
    let executor = AdyenExecutor::new().await;

    for case in generated_input_variants() {
        let mut context = FlowContext::new(case, "adyen_capture_partial_suite");
        authorize::execute(
            &executor,
            "adyen_capture_partial_suite",
            &mut context,
            authorize::default_manual_scenario(),
        )
        .await;
