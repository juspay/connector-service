# Connector `stripe` / Suite `revoke_mandate` / Scenario `revoke_with_reason`
...
x-connector-request-reference-id: revoke_mandate_revoke_with_reason_ref
x-merchant-id: test_merchant
x-request-id: revoke_mandate_revoke_with_reason_req
x-tenant-id: default
...
grpcurl -plaintext \
  -H "x-merchant-id: test_merchant" \
  -H "x-tenant-id: default" \
  -H "x-request-id: revoke_mandate_revoke_with_reason_req" \
  -H "x-connector-request-reference-id: revoke_mandate_revoke_with_reason_ref" \
  -H "x-connector-config: ***MASKED***" \
  -d @ localhost:50051 types.RecurringPaymentService/Revoke <<'JSON'
{
  "merchant_revoke_id": "gen_986136",
  "mandate_id": "gen_388843",
  "connector_mandate_id": "gen_483326"
}
JSON
```
