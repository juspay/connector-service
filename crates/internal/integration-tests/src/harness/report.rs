...
use crate::harness::scenario_loader::{
    discover_all_connectors, load_connector_spec, load_suite_scenarios,
};
...
fn suite_display_name(suite: &str) -> String {
    let name = match suite {
        ...
        other => {
            // New naming: ServiceName/FlowName → extract flow part and insert spaces before capitals
            let flow = other.split('/').nth(1).unwrap_or(other);
...
fn build_suite_service_cache() -> BTreeMap<String, String> {
    ...
    // With the new ServiceName/FlowName naming, the suite name IS the
    // service path.  Discover all suites from connector specs and insert
    // them as identity mappings.
            ...
            if let Some(spec) = load_connector_spec(&connector) {
                for suite in spec.supported_suites {
                    if !cache.contains_key(&suite) {
                        cache.insert(suite.clone(), suite);
                    }
                }
            }
        }
        ...
        let report = ScenarioRunReport {
            runs: vec![
                ...
                ReportEntry {
                    ...
                    dependency: vec!["CustomerService/Create(create_customer)".to_string()],
                    req_body: Some(serde_json::json!({"field": "value"})),
