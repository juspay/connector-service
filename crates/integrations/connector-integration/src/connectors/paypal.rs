...
use crate::{
    connectors::paypal::transformers::{
        ...
        PaypalPaymentsRequest, PaypalPayoutSyncResponse, PaypalRefundRequest,
...
impl<T: PaymentMethodDataTypes + Debug + Sync + Send + 'static + Serialize>
    connector_types::ClientAuthentication for Paypal<T>
{
}
...
// ----------------------------------------------------------------------------
// PayoutTransfer Implementation
// ----------------------------------------------------------------------------

impl<T: PaymentMethodDataTypes + Debug + Sync + Send + 'static + Serialize>
    connector_types::PayoutTransferV2 for Paypal<T>
{
}
...
// ----------------------------------------------------------------------------
// PayoutGet Implementation
// ----------------------------------------------------------------------------

impl<T: PaymentMethodDataTypes + Debug + Sync + Send + 'static + Serialize>
    connector_types::PayoutGetV2 for Paypal<T>
{
}
    ...
    for Paypal<T>
{
    ...
    fn handle_response_v2(
        ...
        ConnectorError,
    > {
        let response: PaypalPayoutSyncResponse = res
            .response
...
impl<T: PaymentMethodDataTypes + Debug + Sync + Send + 'static + Serialize>
    connector_types::ServerSessionAuthentication for Paypal<T>
{
}
impl<T: PaymentMethodDataTypes + Debug + Sync + Send + 'static + Serialize>
    connector_types::ServerAuthentication for Paypal<T>
{
}
impl<T: PaymentMethodDataTypes + Debug + Sync + Send + 'static + Serialize>
    ...
    member_functions: {
     ...
     pub fn get_order_error_response(
        ...
        let response: paypal::PaypalOrderErrorResponse = res
            ...
            .change_context(utils::response_handling_fail_for_connector(res.status_code, "paypal"))
            ?;
                    ...
                    transformers::PaypalPaymentIntent::Authenticate => {
                        format!(
                            ...
                            req.request
                                ...
                                .get_connector_transaction_id()
                                .change_context(
                                    IntegrationError::MissingConnectorTransactionID { context: Default::default() }
                                )?
                        )
                    }
                };
