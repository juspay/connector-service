...
macros::macro_connector_implementation!(
    ...
    other_functions: {
        fn get_headers(
            ...
            let mut header = self.get_auth_header(&req.connector_config)?;
            header.push((headers::CONTENT_TYPE.to_string(), self.common_get_content_type().to_string().into()));
            Ok(header)
        }
        fn get_url(
            &self,
            ...
            Ok(format!("{}/v1/sessions", self.base_url(&req.resource_common_data.connectors)))
        }
    }
);
