...
/// This macro works identically to [`macro_connector_payout_implementation!`] but for surcharge flows.
/// It emits:
/// 1. Marker trait implementation (e.g., `SurchargeCalculateV2`)
/// 2. Empty `ConnectorIntegrationV2` implementation (to be overridden by connectors that support it)
///
/// # Usage
///
/// ```rust
...
///     surcharge_flows: [SurchargeCalculate]
/// );
/// ```
macro_rules! macro_connector_surcharge_implementation {
    // Arm 1: Default – no explicit surcharge_flows list provided.
    // Supplies the default set of surcharge flows and re-invokes itself.
    (
        connector: $connector: ident,
        generic_type: $generic_type:tt,
        [ $($bounds:tt)* ]
    ) => {
        $crate::connectors::macros::macro_connector_surcharge_implementation!(
            connector: $connector,
            generic_type: $generic_type,
            [ $($bounds)* ],
            surcharge_flows: [SurchargeCalculate]
    ...
    // Arm 2: Recursive – peel the first flow (`$flow`) from the list, generate its impls
    // via `expand_surcharge_implementation!`, then recurse on the remaining flows (`$rest`).
    (
        connector: $connector: ident,
        generic_type: $generic_type:tt,
        [ $($bounds:tt)* ],
        surcharge_flows: [ $flow: ident $(, $rest: ident)* ]
    ) => {
        $crate::connectors::macros::expand_surcharge_implementation!(
            connector: $connector,
            flow: $flow,
            generic_type: $generic_type,
            [ $($bounds)* ]
...
pub(crate) use macro_connector_surcharge_implementation;

/// Emits the concrete trait implementations for a **single** surcharge flow on a connector.
///
/// This macro is the internal workhorse called by [`macro_connector_surcharge_implementation!`].
/// It pattern-matches on the flow identifier and generates two trait implementations:
///
/// 1. **Marker trait** (`SurchargeCalculateV2`) from `::interfaces::connector_types`
/// 2. **Integration trait** (`ConnectorIntegrationV2<Flow, FlowData, Request, Response>`)
///    with a default empty implementation (to be overridden by connectors that support it)
macro_rules! expand_surcharge_implementation {
    (
        connector: $connector: ident,
        flow: SurchargeCalculate,
        generic_type: $generic_type:tt,
        [ $($bounds:tt)* ]
    ) => {
        impl<$generic_type: $($bounds)* + ::domain_types::payment_method_data::PaymentMethodDataTypes> ::interfaces::connector_types::SurchargeCalculateV2 for $connector<$generic_type> {}

        impl<$generic_type: $($bounds)* + ::domain_types::payment_method_data::PaymentMethodDataTypes>
            ::interfaces::connector_integration_v2::ConnectorIntegrationV2<
                ::domain_types::connector_flow::SurchargeCalculate,
                ::domain_types::surcharge::surcharge_types::SurchargeFlowData,
                ::domain_types::surcharge::surcharge_types::SurchargeCalculateRequest,
                ::domain_types::surcharge::surcharge_types::SurchargeCalculateResponse,
            > for $connector<$generic_type>
        {}
    };
}
pub(crate) use expand_surcharge_implementation;
