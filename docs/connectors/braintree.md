...
config = sdk_config_pb2.ConnectorConfig(
    connector=sdk_config_pb2.Connector.BRAINTREE,
...
async def main():
    client = PaymentMethodClient(config)
    response = await client.tokenize(request)
    print(response)
...
const response = await client.tokenize(request);
console.log(response);
```
...
```rust
use connector_service_sdk::{ConnectorClient, ConnectorConfig};
use grpc_api_types::payments::PaymentMethodServiceTokenizeRequest;
...
pub async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Field names and descriptions from the proto definition above
    let request = PaymentMethodServiceTokenizeRequest {
        // amount: todo!(),  // Payment Information
        // payment_method: todo!(),
        // customer: todo!(),  // Customer Information
        // address: todo!(),  // Address Information
        ..Default::default()
    ...
    let client = ConnectorClient::new(config, None)?;
    let response = client.tokenize(request, &Default::default(), None).await?;
    println!("{response:?}");
    Ok(())
}
```
...
<!-- TODO: Add sample payload for `authenticate` in `scripts/connector-annotations/braintree.yaml` -->
...
<!-- TODO: Add sample payload for `post_authenticate` in `scripts/connector-annotations/braintree.yaml` -->
...
<!-- TODO: Add sample payload for `pre_authenticate` in `scripts/connector-annotations/braintree.yaml` -->
