...
config = sdk_config_pb2.ConnectorConfig(
    connector=sdk_config_pb2.Connector.TRUSTPAY,
...
```kotlin
import payments.PaymentClient
import types.Payment.PaymentServiceCreateOrderRequest
import com.google.protobuf.util.JsonFormat
...
val builder = PaymentServiceCreateOrderRequest.newBuilder()
JsonFormat.parser().ignoringUnknownFields().merge(json, builder)
val request = builder.build()
...
```rust
use connector_service_sdk::{ConnectorClient, ConnectorConfig};
use grpc_api_types::payments::PaymentServiceCreateOrderRequest;
...
pub async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let request = PaymentServiceCreateOrderRequest {
        // merchant_order_id: todo!(),  // Identification
    ...
    let client = ConnectorClient::new(config, None)?;
    let response = client.create_order(request, &Default::default(), None).await?;
    println!("{response:?}");
    Ok(())
}
```
...
<!-- TODO: Add sample payload for `authenticate` in `scripts/connector-annotations/trustpay.yaml` -->
...
<!-- TODO: Add sample payload for `post_authenticate` in `scripts/connector-annotations/trustpay.yaml` -->
...
<!-- TODO: Add sample payload for `pre_authenticate` in `scripts/connector-annotations/trustpay.yaml` -->
