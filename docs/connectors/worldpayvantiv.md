...
config = sdk_config_pb2.ConnectorConfig(
    connector=sdk_config_pb2.Connector.WORLDPAYVANTIV,
...
async def main():
    client = PaymentClient(config)
    response = await client.reverse(request)
    print(response)
...
const response = await client.reverse(request);
console.log(response);
```
...
```kotlin
import payments.PaymentClient
import types.Payment.PaymentServiceReverseRequest
import com.google.protobuf.util.JsonFormat
...
val builder = PaymentServiceReverseRequest.newBuilder()
JsonFormat.parser().ignoringUnknownFields().merge(json, builder)
val request = builder.build()
...
```rust
use connector_service_sdk::{ConnectorClient, ConnectorConfig};
use grpc_api_types::payments::PaymentServiceReverseRequest;
...
pub async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Field names and descriptions from the proto definition above
    let request = PaymentServiceReverseRequest {
        // merchant_reverse_id: todo!(),  // Identification
        // connector_transaction_id: todo!(),
        ..Default::default()
    };

    let client = ConnectorClient::new(config, None)?;
    let response = client.reverse(request, &Default::default(), None).await?;
    println!("{response:?}");
    Ok(())
}
```
...
<!-- TODO: Add sample payload for `authenticate` in `scripts/connector-annotations/worldpayvantiv.yaml` -->
...
<!-- TODO: Add sample payload for `post_authenticate` in `scripts/connector-annotations/worldpayvantiv.yaml` -->
...
<!-- TODO: Add sample payload for `pre_authenticate` in `scripts/connector-annotations/worldpayvantiv.yaml` -->
