...
use grpc_api_types::payments::{Connector, ConnectorConfig, Environment, MerchantAuthenticationServiceCreateSessionTokenRequest};
use hyperswitch_payments_client::ConnectorClient;

#[tokio::main]
async fn main() {
    let config = ConnectorConfig {
        connector: Connector::Paytm.into(),
    ...
    let response = client.create_session_token(request).await.unwrap();
