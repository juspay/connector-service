...
fn build_client() -> ConnectorClient {
    let config = ConnectorConfig {
        connector: Connector::Forte.into(),
...
async fn main() {
    ...
    let result: Result<String, Box<dyn std::error::Error>> = match flow.as_str() {
        ...
        _ => { eprintln!("Unknown flow: {}. Available: process_checkout_autocapture, process_checkout_bank, process_void_payment, process_get_payment, authorize, get, void", flow); return; }
    };
    match result {
        Ok(msg) => println!("✓ {msg}"),
        Err(e) => eprintln!("✗ {e}"),
