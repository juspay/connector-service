...
fun submit_evidence(txnId: String) {
    val client = DisputeClient(_defaultConfig)
...
fun main(args: Array<String>) {
    ...
    when (flow) {
        ...
        "accept" -> accept(txnId)
        "defend" -> defend(txnId)
        "submit_evidence" -> submit_evidence(txnId)
        "charge" -> charge(txnId)
        "refund" -> refund(txnId)
        "setup_recurring" -> setup_recurring(txnId)
        "void" -> void(txnId)
        else -> System.err.println("Unknown flow: $flow. Available: processCheckoutCard, processCheckoutAutocapture, processCheckoutWallet, processCheckoutBank, processRefund, processRecurring, processVoidPayment, authorize, capture, accept, defend, submit_evidence, charge, refund, setup_recurring, void")
    }
}
