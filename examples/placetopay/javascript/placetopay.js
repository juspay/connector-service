...
// Run a scenario:  node placetopay.js checkout_card
'use strict';
...
//     placetopay: { apiKey: { value: 'YOUR_API_KEY' } }
// });
...
async function capture(merchantTransactionId, config = _defaultConfig) {
    ...
    const captureResponse = await paymentClient.capture(_buildCaptureRequest('12345'));
...
async function voidPayment(merchantTransactionId, config = _defaultConfig) {
    ...
    const voidResponse = await paymentClient.void(_buildVoidRequest('12345'));

    return { status: voidResponse.status };
}


module.exports = { processCheckoutCard, processCheckoutAutocapture, processRefund, processVoidPayment, processGetPayment, authorize, capture, get, refund, voidPayment, _buildAuthorizeRequest, _buildCaptureRequest, _buildGetRequest, _buildRefundRequest, _buildVoidRequest };
