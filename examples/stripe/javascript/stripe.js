...
const { DirectPaymentClient, RecurringPaymentClient, CustomerClient, PaymentMethodClient } = require('hs-playlib');
...
async function processCheckoutCard(merchantTransactionId, config = _defaultConfig) {
    const paymentClient = new DirectPaymentClient(config);
...
async function processRecurring(merchantTransactionId, config = _defaultConfig) {
    const paymentClient = new DirectPaymentClient(config);
    const recurringDirectPaymentClient = new RecurringPaymentClient(config);
    ...
    const recurringResponse = await recurringDirectPaymentClient.charge({
