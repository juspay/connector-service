...
fun processCreateCustomer(txnId: String, config: ConnectorConfig): Map<String, Any?> {
    // Create Customer
    val customerClient = CustomerClient(config)

    // Step 1: Create Customer — register customer record in the connector
    val result = customerClient.create(CustomerServiceCreateRequest.newBuilder().setMerchantCustomerId("cust_probe_123").setCustomerName("John Doe").setEmail(SecretString.newBuilder().setValue("test@example.com").build()).setPhoneNumber("4155552671").build())
...
fun create_customer(txnId: String, config: ConnectorConfig): Map<String, Any?> {
    // Flow: CustomerService.create_customer
    val customerClient = CustomerClient(config)
