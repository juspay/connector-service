...
pub fn build_authenticate_request() -> PaymentMethodAuthenticationServiceAuthenticateRequest {
    PaymentMethodAuthenticationServiceAuthenticateRequest {
        ...
        redirection_response: Some(RedirectionResponse {  // Redirection Information after DDC step.
            params: Some("probe_redirect_params".to_string()),
            payload: [("transaction_id".to_string(), "probe_txn_123".to_string())].into_iter().collect::<HashMap<_, _>>(),
            ..Default::default()
        }),
        ..Default::default()
    }
...
#[allow(dead_code)]
pub async fn process_post_authenticate(client: &ConnectorClient, _merchant_transaction_id: &str) -> Result<String, Box<dyn std::error::Error>> {
    let response = client.post_authenticate(build_post_authenticate_request(), &HashMap::new(), None).await?;
    Ok(format!("status: {:?}", response.status()))
}
...
#[allow(dead_code)]
pub async fn process_recurring_revoke(client: &ConnectorClient, _merchant_transaction_id: &str) -> Result<String, Box<dyn std::error::Error>> {
...
async fn main() {
    ...
    let result: Result<String, Box<dyn std::error::Error>> = match flow.as_str() {
        ...
        _ => { eprintln!("Unknown flow: {}. Available: process_checkout_autocapture, process_checkout_card, process_refund, process_void_payment, process_get_payment, process_authenticate, process_authorize, process_capture, process_get, process_post_authenticate, process_pre_authenticate, process_proxy_authorize, process_recurring_charge, process_recurring_revoke, process_refund_get, process_token_authorize, process_void", flow); return; }
    };
    match result {
        Ok(msg) => println!("✓ {msg}"),
        Err(e) => eprintln!("✗ {e}"),
