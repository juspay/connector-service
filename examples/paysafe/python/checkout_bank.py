...
_default_config = sdk_config_pb2.ConnectorConfig(
    connector=payment_pb2.Connector.PAYSAFE,
    environment=sdk_config_pb2.Environment.SANDBOX,
)
# Standalone credentials (field names depend on connector auth type):
# _default_config.auth.paysafe.api_key.value = "YOUR_API_KEY"
    ...
    authorize_response = await payment_client.authorize(ParseDict(
        {
            ...
            "payment_method_token": {"value": "probe_session_token"}  # Payment Method Token
        },
...
if __name__ == "__main__":
    asyncio.run(process_checkout_bank("order_001"))
