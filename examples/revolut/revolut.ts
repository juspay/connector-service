...
//     revolut: { apiKey: { value: 'YOUR_API_KEY' } }
// };
...
async function tokenAuthorize(merchantTransactionId: string, config: ConnectorConfig = _defaultConfig): Promise<any> {
...
// Flow: PaymentService.verify_redirect
async function verifyRedirect(merchantTransactionId: string, config: ConnectorConfig = _defaultConfig): Promise<any> {
    // Step 1: verify_redirect
    const verifyResponse = await paymentClient.verifyRedirect({
        // No required fields
    });
...
export {
    processCheckoutAutocapture, processCheckoutCard, processRefund, processGetPayment, authorize, capture, get, handleEvent, proxyAuthorize, refund, refundGet, tokenAuthorize, verifyRedirect
};
