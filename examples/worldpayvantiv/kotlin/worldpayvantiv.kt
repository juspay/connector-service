...
fun reverse(txnId: String, config: ConnectorConfig): Map<String, Any?> {
    // Flow: PaymentService.reverse
    val directPaymentClient = DirectPaymentClient(config)

    val result = directPaymentClient.reverse(PaymentServiceReverseRequest.newBuilder().setMerchantReverseId("probe_reverse_001").setConnectorTransactionId("probe_connector_txn_001").build())
    return mapOf("status" to "ok")
}
