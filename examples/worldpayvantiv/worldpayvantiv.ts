...
async function reverse(merchantTransactionId: string, config: ConnectorConfig = _defaultConfig): Promise<any> {
    // Step 1: reverse
    const reverseResponse = await paymentClient.reverse({
        "merchantReverseId": "probe_reverse_001",
        "connectorTransactionId": "probe_connector_txn_001"
    });
...
export {
    processCheckoutAutocapture, processCheckoutCard, processRefund, processVoidPayment, processGetPayment, authorize, capture, get, proxyAuthorize, refund, refundGet, reverse, voidPayment
};
