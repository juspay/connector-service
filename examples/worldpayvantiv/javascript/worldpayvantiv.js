...
// Run a scenario:  node worldpayvantiv.js checkout_card
'use strict';
...
//     worldpayvantiv: { apiKey: { value: 'YOUR_API_KEY' } }
// });
...
async function reverse(merchantTransactionId, config = _defaultConfig) {
    const paymentClient = new PaymentClient(config);

    const reverseResponse = await paymentClient.reverse(_buildReverseRequest());
...
module.exports = { processCheckoutCard, processCheckoutAutocapture, processRefund, processVoidPayment, processGetPayment, authorize, capture, get, proxyAuthorize, refund, reverse, voidPayment, _buildAuthorizeRequest, _buildCaptureRequest, _buildGetRequest, _buildRefundRequest, _buildReverseRequest, _buildVoidRequest };
